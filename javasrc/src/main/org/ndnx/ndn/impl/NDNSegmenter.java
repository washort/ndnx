...
import org.ndnx.ndn.impl.security.crypto.NDNAggregatedSigner;
import org.ndnx.ndn.impl.security.crypto.NDNMerkleTree;
import org.ndnx.ndn.impl.security.crypto.NDNMerkleTreeSigner;
import org.ndnx.ndn.impl.security.crypto.ContentKeys;
import org.ndnx.ndn.impl.security.crypto.UnbufferedCipherInputStream;
 ...
 * to prepare data for writing out to ndnd. The intent is to provide a user-friendly,
 ...
 *   @see org.ndnx.ndn.profiles.SegmentationProfile
 *
 ...
 *    by the higher level org.ndnx.ndn.io interfaces.
 *
 ...
 *    @see org.ndnx.ndn.impl.security.crypto.NDNMerkleTreeSigner</li>
 *
...
public class NDNSegmenter {
	 ...
	 */
	protected NDNAggregatedSigner _bulkSigner;
	 ...
	 */
	public NDNSegmenter() throws ConfigurationException, IOException {
		this(NDNHandle.open());
	}
	 ...
	 * @throws IOException if there is a problem creating the handle
	 */
	public NDNSegmenter(NDNHandle handle) throws IOException {
		this(new NDNFlowControl(handle));
	}
	 ...
	 */
	public NDNSegmenter(NDNFlowControl flowControl) {
	 ...
	 */
	public NDNSegmenter(NDNFlowControl flowControl, NDNAggregatedSigner signer) {
		if ((null == flowControl) || (null == flowControl.getHandle())) {
			// Tries to get a library or make a flow control, yell if we fail.
			throw new IllegalArgumentException("NDNSegmenter: must provide a valid library or flow controller.");
		}
		_flowControl = flowControl;
		_handle = _flowControl.getHandle();
		if (null == signer) {
			_bulkSigner = new NDNMerkleTreeSigner();
		} else {
	 ...
	 */
	public static NDNSegmenter getBlockSegmenter(int blockSize, NDNFlowControl flowControl) {
		NDNSegmenter segmenter = new NDNSegmenter(flowControl);
	 ...
	 * @return the new segmenter
	 */
	public static NDNSegmenter getScaledByteCountSegmenter(int scale, NDNFlowControl flowControl) {
		NDNSegmenter segmenter = new NDNSegmenter(flowControl);
	 ...
	 * NOTE: This is used by NDNBlockInputStream and NDNBlockOutputStream; the other stream classes
	 *   will not read data generated this way.
	 * @param flowControl the flow controller to use
	 * @return the new segmenter
	 */
	public static NDNSegmenter getByteCountSegmenter(NDNFlowControl flowControl) {
		return getScaledByteCountSegmenter(1, flowControl);
	}

	public NDNHandle getLibrary() { return _handle; }

	public NDNFlowControl getFlowControl() { return _flowControl; }
	...
	throws InvalidKeyException, SignatureException, NoSuchAlgorithmException, IOException, InvalidAlgorithmParameterException {
		...
		if (outputLength(length, keys) >= getBlockSize()) {
			return fragmentedPut(name, content, offset, length, (lastSegments ? NDNSegmenter.LAST_SEGMENT : null),
	...
	public long fragmentedPut(
		...
		// This will call into NDNBase after picking appropriate credentials
		...
		if (null != finalSegmentIndex) {
			if (finalSegmentIndex.longValue() == NDNSegmenter.LAST_SEGMENT) {
				// compute final segment number
				// compute final segment number; which might be this one if blockCount == 1
				int blockCount = NDNMerkleTree.blockCount(length, blockWidth);
	...
	protected void outputCurrentBlocks(Key signingKey) throws InvalidKeyException, SignatureException, NoSuchAlgorithmException, IOException {
		...
		if (_blocks.size() == 1) {
			...
			if( Log.isLoggable(Level.FINER))
				Log.finer("NDNSegmenter: putting " + co.name() + " (timestamp: " + co.signedInfo().getTimestamp() + ", length: " + co.contentLength() + ")");
			_flowControl.put(co);
