 ...
 * for content objects.
 *
 * ndnd will not accept data (content objects) except in response to an
 ...
 * arrive from ndnd. Equally when interests come in from ndnd before the
 ...
 */
public class NDNFlowControl implements NDNInterestHandler {
	...
	// Designed to allow a NDNOutputStream to flush its current output once without
	// causing the over-capacity blocking to be triggered
	protected static final int DEFAULT_CAPACITY = NDNSegmenter.HOLD_COUNT + 1;
	...
	//TODO need to normalize this with refresh time in NDNNetworkManager and put in SystemConfiguration
	 ...
	 * @param handle	NDNHandle - created if null
	 * @throws IOException if handle can't be created
	 */
	public NDNFlowControl(ContentName name, NDNHandle handle) throws IOException {
		this(handle);
		if (name != null) {
	 ...
	 * @throws IOException	if handle can't be created
	 */
	public NDNFlowControl(String name, NDNHandle handle)
	 ...
	 * @throws IOException	if handle can't be created
	 */
	public NDNFlowControl(NDNHandle handle) throws IOException {
		if (null == handle) {
			try {
	 ...
	 * this Interest is only handed to one NDNFlowControl to emit a block.
	 ...
	 */
	public NDNFlowControl(ContentName name, Interest outstandingInterest, NDNHandle handle) throws IOException {
		this(name, handle);
		handleInterest(outstandingInterest);
	}

	/**
	 * Add a new namespace to the controller. The controller will register a filter with ndnd to receive
	...
	/**
	 * Add a content object to this flow controller. It won't be sent to ndnd immediately unless
	...
	/**
	 * Allow override of action after a ContentObject is sent to ndnd
	 *
		...
		synchronized (_holdingArea) {
			...
			while (_holdingArea.size() > 0) {
				...
				if (_nOut == startSize) {
					...
					NDNNetworkManager cnm = _handle.getNetworkManager();
					if (null != cnm)
	...
	/**
	 * Gets the NDNHandle used by this controller
	 * @return	a NDNHandle
	 */
