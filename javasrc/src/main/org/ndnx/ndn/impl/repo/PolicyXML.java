...
public class PolicyXML extends GenericXMLEncodable implements XMLEncodable {
	...
	public static class PolicyObject extends NDNEncodableObject<PolicyXML> {
		
		/**
		 * Read constructor
		 */
		public PolicyObject(ContentName name, NDNHandle handle) 
		...
		public PolicyObject(ContentName name,
				PolicyXML data, SaveType saveType, NDNHandle handle)
				throws IOException {
	...
	private enum PolicyElement {
		VERSION (NDNProtocolDTags.PolicyVersion, new VersionPutter()),
		NAMESPACE (NDNProtocolDTags.Namespace, new NameSpacePutter()),
		LOCALNAME (NDNProtocolDTags.LocalName, new LocalNamePutter()),
		GLOBALPREFIX (NDNProtocolDTags.GlobalPrefix, new GlobalPrefixPutter());
	...
	public void encode(XMLEncoder encoder) throws ContentEncodingException {
		...
		encoder.writeElement(NDNProtocolDTags.PolicyVersion, _version);	
		encoder.writeElement(NDNProtocolDTags.LocalName, _localName);
		encoder.writeElement(NDNProtocolDTags.GlobalPrefix, _globalPrefix.toString());
		
		if (null != _namespace) {
			synchronized (_namespace) {
				for (ContentName name : _namespace)
					encoder.writeElement(NDNProtocolDTags.Namespace, name.toString());
			}
		}
	...
	public long getElementLabel() {
		return NDNProtocolDTags.Policy;
	}
