 ...
 * "write_marker" (NDNBase.REPO_START_WRITE). When we see this we write back
...
public class RepositoryServer implements NDNStatistics {
	private final int REPO_PREFIX_FLAGS = PrefixRegistrationManager.DEFAULT_SELF_REG_FLAGS
				+ PrefixRegistrationManager.NDN_FORW_TAP;
	private RepositoryStore _repo = null;
	private NDNHandle _handle = null;
	...
	private final NDNWriter _writer;
	 ...
	 * the second handler sends a redundant duplicate answer back to ndnd which has a quite dramatic negative
	...
	protected NDNEnumStats<StatsEnum> _stats = new NDNEnumStats<StatsEnum>(StatsEnum.HandleInterest);
