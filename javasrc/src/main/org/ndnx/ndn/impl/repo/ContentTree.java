...
public class ContentTree {
					...
					if (node.interestFlag && (ner != null && ner.getPrefix()==null)){
						...
						if (Log.isLoggable(Log.FAC_REPO, Level.INFO)) {
							...
							Log.info(Log.FAC_REPO, "response name will be: {0}",
									new ContentName(prefix, COMMAND_MARKER_BASIC_ENUMERATION, new NDNTime(node.timestamp)));
						}
						...
						ner.setTimestamp(new NDNTime(node.timestamp));
						if (Log.isLoggable(Log.FAC_REPO, Level.INFO)) {
	...
	public final NameEnumerationResponse getNamesWithPrefix(Interest interest, ContentName responseName) {
		...
		if (parent!=null) {
			...
			NDNTime timestamp = new NDNTime(parent.timestamp);		// I think we want to use the earliest possible timestamp here - if there are duplicates
