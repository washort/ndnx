 ...
 */
public class NDNNetworkChannel extends InputStream {
...
//	public static final int DOWN_DELAY = SystemConfiguration.MEDIUM_TIMEOUT;	// Wait period for retry when ndnd is down
	...
	protected ByteBuffer _datagram = ByteBuffer.allocateDirect(NDNNetworkManager.MAX_PAYLOAD);
	...
	//private byte[] buffer = new byte[NDNNetworkManager.MAX_PAYLOAD];
	//protected ByteBuffer _datagram = ByteBuffer.wrap(buffer);
	private int _mark = -1;
	private int _readLimit = 0;
	private int _lastMark = 0;

	public NDNNetworkChannel(String host, int port, NetworkProtocol proto, FileOutputStream tapStreamIn) throws IOException {
		_ncHost = host;
		_ncPort = port;
		_ncProto = proto;
		...
		if (Log.isLoggable(Log.FAC_NETMANAGER, Level.INFO))
			Log.info(Log.FAC_NETMANAGER, "NetworkChannel {0}: Starting up NDNNetworkChannel using {1}.",  _channelId, proto.toString());
	}

	/**
	 * Open the channel to ndnd depending on the protocol, connect on the ndnd port and
	 * set up the selector
	 *
	 * @throws IOException
	 */
		...
		synchronized(_opencloseLock) {
			...
			if (_ncProto == NetworkProtocol.UDP) {
				try {
					...
					// For some weird reason we seem to have to test writing twice when ndnd is down
			...
			if (Log.isLoggable(Log.FAC_NETMANAGER, Level.INFO)) {
				...
				Log.info(Log.FAC_NETMANAGER, "NetworkChannel {0}: {1} NDN agent at {2}:{3} on local port {4}",
						_channelId,
						connecting,
						_ncHost,
						_ncPort,
	...
	/**
	 * Get the next packet from the network. It could be either an interest or data. If ndnd is
	 ...
	 *
	 * Also it should be noted that we are relying on ndnd to guarantee that all packets sent
	 * to us are complete ndn packets. This code does not have the ability to recover from
	 * receiving a partial ndn packet followed by correctly formed ones.
	 *
	 ...
	 * to see whether ndnd is running. If it isn't the channel is not connected.
	...
	/**
	 * Write to ndnd using methods based on the protocol type
