...
public class FileBasedSyncMonitor extends SyncMonitor implements Runnable{
	...
	public FileBasedSyncMonitor() throws ConfigurationException{
		...
		synchronized (runningLock) {
			...
			if (!isRunning) {
				//based on the env variable for running ndnr
				filename = System.getenv("NDNR_DIRECTORY");
				if (filename == null) {
					Log.severe("Please set NDNR_DIRECTORY environment variable before running!");
					throw new ConfigurationException("Please set NDNR_DIRECTORY environment variable before running!");
				}

				String configError = "Please install the NDNx installation or set your path to use the ndnnamelist command";
				try {
					Log.fine(Log.FAC_SYNC, "PATH: {0}", System.getenv("PATH"));
					ProcessBuilder pbCheck = new ProcessBuilder();
					pbCheck.command("/bin/sh", "-c", "command -v ndnnamelist");
					int exitStatus = pbCheck.start().waitFor();
					if (exitStatus == 0) {
						//the command is found!
						Log.fine(Log.FAC_SYNC, "ndnnamelist command was found in the path!");
					} else {
	...
	public void run() {
		...
		String commandCreateDiff= "ndnnamelist repoFile1 > names ;  sort names > newnames ; rm names ; :>> oldnames ; diff newnames oldnames > diffNames ; mv newnames oldnames";
		...
		while (keepRunning) {
			...
			try {
				...
				Log.severe("NDNR_DIRECTORY setting = {0}: file not found.  Exiting.", filename);
				System.exit(1);
			} catch (IOException e) {
				try {
					fileLock.release();
	...
	private void processCallback(NDNSyncHandler syncHandler, ConfigSlice slice) {
		synchronized(callbacks) {
	...
	private void processRemoveCallback(NDNSyncHandler syncHandler, ConfigSlice slice) {
		synchronized(callbacks) {
	...
	public void registerCallback(NDNSyncHandler syncHandler, ConfigSlice slice, byte[] startHash, ContentName startName) {
		//start monitoring the namespace, if not already monitored...  if so, add to the callback list
		processCallback(syncHandler, slice);
	}
	
	public void removeCallback(NDNSyncHandler syncHandler, ConfigSlice slice) {
	...
	public void shutdown(ConfigSlice slice) {
		...
		for (NDNSyncHandler syncHandler : cb)
