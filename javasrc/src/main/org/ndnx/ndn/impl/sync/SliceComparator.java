...
import org.ndnx.ndn.impl.sync.SyncNodeCache.Pending;
...
public class SliceComparator implements Runnable {
	...
	protected ArrayList<NDNSyncHandler> _callbacks = new ArrayList<NDNSyncHandler>();
	protected ArrayList<NDNSyncHandler> _pendingCallbacks = new ArrayList<NDNSyncHandler>();
	 ...
	 * @param handle the NDNHandle to use
	 */
	public SliceComparator(SliceComparator leadComparator, SyncNodeCache snc, NDNSyncHandler callback, ConfigSlice slice, 
				byte[] startHash, ContentName startName, NDNHandle handle) {
	 ...
	 * @param hash
	 */
	public void addCallback(NDNSyncHandler callback) {
		synchronized (this) {
	 ...
	 * @return
	 */
	public synchronized ArrayList<NDNSyncHandler> getCallbacks() {
		return _callbacks;
	}
	 ...
	 * @return the callback
	 */
	public void removeCallback(NDNSyncHandler callback) {
		synchronized (this) {
	...
	private void newName(SyncNodeComposite.SyncNodeElement sne) {
		...
		for (NDNSyncHandler callback : _callbacks)
		...
		try {
			do {
				...
				case INIT:		// Starting a new compare
					...
					synchronized (this) {
						for (NDNSyncHandler callback : _pendingCallbacks) {
							_callbacks.add(callback);
						}
					...
					if (this != _leadComparator && didARound) {
						...
						synchronized (_leadComparator) {
							...
							if (doSwitch) {
								...
								// worry about it.
								for (NDNSyncHandler callback : _callbacks)	// There should never really be more than one here			
	 ...
	 */
	protected class NodeFetchHandler implements NDNContentHandler {
