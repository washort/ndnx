...
import static org.ndnx.ndn.impl.support.Log.FAC_KEYS;
 ...
 * - answers NDN interestes for these public keys.
 * - allow the caller to look for public keys: retrieve the keys from cache or NDN 
 * 
 ...
 * the library. So we allow the KeyRepository to have a NDNHandle, we can use
...
public class PublicKeyCache {
	...
	protected HashMap<PublisherPublicKeyDigest, NDNTime> _rawVersionMap = new HashMap<PublisherPublicKeyDigest, NDNTime>();
	 ...
	 * @param theKey public key to remember
	 */
	public void remember(PublicKey theKey, NDNTime version) {
	 ...
	 */
	public void remember(Certificate theCertificate, NDNTime version) {
	...
	protected void recordKeyToFile(PublicKeyObject keyObject) throws ContentNotReadyException, ContentGoneException, ErrorStateException {
		...
		if (!keyDir.exists()) {
			if (!keyDir.mkdirs()) {
				Log.warning(FAC_KEYS, "recordKeyToFile: Cannot create user NDN key repository directory: {0}", keyDir.getAbsolutePath());
				return;
			}
		}
	...
	/**
	 * Retrieve the public key from NDN given a key digest and a key locator
	...
	public PublicKey getPublicKey(
			...
			long timeout, NDNHandle handle) throws IOException {
	...
	public NDNTime getPublicKeyVersionFromCache(PublisherPublicKeyDigest desiredKeyID) {
