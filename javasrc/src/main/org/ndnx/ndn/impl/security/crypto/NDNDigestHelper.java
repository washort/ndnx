...
/**
 * NDN-specific helper methods for working with digests, primarily to support Merkle trees.
 */
public class NDNDigestHelper extends DigestHelper {
	...
	/**
	 * Make a NDNDigestHelper using the default digest algorithm (DEFAULT_DIGEST_ALGORITHM).
	 */
	public NDNDigestHelper() {
		super();
		if (!_md.getAlgorithm().equals(NDNDigestHelper.DEFAULT_DIGEST_ALGORITHM)) {
			Log.severe("Incorrect constructor in NDNDigestHelper -- picking up wrong digest algorithm!");
		}
	}

	/**
	 * Make a NDNDigestHelper that uses a specified algorithm.
	 ...
	 */
	public NDNDigestHelper(String digestAlgorithm) throws NoSuchAlgorithmException {
	...
	public static byte [] digestLeaf(
		...
		return DigestHelper.digest(((null == digestAlgorithm) || (digestAlgorithm.length() == 0)) ?
				NDNDigestHelper.DEFAULT_DIGEST_ALGORITHM : digestAlgorithm, 
				encodedData);
	}
	...
	public static byte [] digest(byte [] content, int offset, int length) {
		NDNDigestHelper dh = new NDNDigestHelper();
	...
	public static byte [] digest(String digestAlgorithm, byte [] content, int offset, int length) throws NoSuchAlgorithmException {
		NDNDigestHelper dh = new NDNDigestHelper(digestAlgorithm);
		dh.update(content, offset, length);
		return dh.digest();
	}
	...
	public static byte [] encodedDigest(byte [] content) throws CertificateEncodingException {
		...
		return digestEncoder(NDNDigestHelper.DEFAULT_DIGEST_ALGORITHM, digest);
	}
}
