 ...
 *     
 */
public class NDNMerkleTree extends MerkleTree {
	...
	/**
	 * Build a NDNMerkleTree from a set of leaf ContentObjects. 
	 ...
	 */
	public NDNMerkleTree(ContentObject [] contentObjects, 
						 Key signingKey) throws NoSuchAlgorithmException, InvalidKeyException, SignatureException {

		super(NDNDigestHelper.DEFAULT_DIGEST_ALGORITHM, ((null != contentObjects) ? contentObjects.length : 0));
		...
		if (Log.isLoggable(Log.FAC_SIGNING, Level.FINE))
			Log.fine(Log.FAC_SIGNING, "NDNMerkleTree: built a tree of " + contentObjects.length + " objects.");
	}
	...
	/**
	 * A version of initializeTree to go with the NDNMerkleTree(ContentObject []) constructor.
	...
	protected static byte [] computeRootSignature(byte [] root, Key signingKey) throws InvalidKeyException, SignatureException, NoSuchAlgorithmException {
		...
		// the sig alg used.
		return NDNSignatureHelper.sign(null, root, signingKey);
	}
