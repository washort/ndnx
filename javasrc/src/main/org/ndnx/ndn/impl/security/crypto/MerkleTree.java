...
public class MerkleTree {
	 ...
	 * 	  with digestAlgorithm? (default algorithm: NDNDigestHelper#DEFAULT_DIGEST_ALGORITHM)
	...
	public MerkleTree(byte [] content, int offset, int length, int blockWidth) {
		this(NDNDigestHelper.DEFAULT_DIGEST_ALGORITHM, blockCount(length, blockWidth));
		try {
			initializeTree(content, offset, length, blockWidth);
		} catch (NoSuchAlgorithmException e) {
			// DKS --big configuration problem
			Log.warning("Fatal Error: cannot find default algorithm " + NDNDigestHelper.DEFAULT_DIGEST_ALGORITHM);
			throw new RuntimeException("Error: can't find default algorithm " + NDNDigestHelper.DEFAULT_DIGEST_ALGORITHM + "!  " + e.toString());
		}
	}
	...
	public MerkleTree(byte contentBlocks[][], boolean isDigest, 
		...
		this(NDNDigestHelper.DEFAULT_DIGEST_ALGORITHM, blockCount);		
		try {
			initializeTree(contentBlocks, isDigest, baseBlockIndex, lastBlockLength);
		} catch (NoSuchAlgorithmException e) {
			// DKS --big configuration problem
	 ...
	 * @param digestAlgorithm digest algorithm to use. If null, use NDNDigestHelper#DEFAULT_DIGEST_ALGORITHM.
	...
	protected MerkleTree(String digestAlgorithm, int numLeaves) {
		...
		_digestAlgorithm = (null == digestAlgorithm) ? NDNDigestHelper.DEFAULT_DIGEST_ALGORITHM : digestAlgorithm;
	...
	public byte[] getRootAsEncodedDigest() {
		...
		return NDNDigestHelper.digestEncoder(
				digestAlgorithm(), 
				root());
	}
	...
	protected byte [] computeBlockDigest(int leafIndex, byte [] content, int offset, int length) throws NoSuchAlgorithmException {
		return NDNDigestHelper.digest(_digestAlgorithm, content, offset, length);		
	}
	...
	public static byte [] computeBlockDigest(String digestAlgorithm, byte [] content) throws NoSuchAlgorithmException {
		return NDNDigestHelper.digest(digestAlgorithm, content);		
	}
	...
	public static byte [] computeBlockDigest(String digestAlgorithm, byte [] content, int offset, int length) throws NoSuchAlgorithmException {
		return NDNDigestHelper.digest(digestAlgorithm, content, offset, length);		
	}

	/**
	 * Compute the digest of a block using NDNDigestHelper#DEFAULT_DIGEST_ALGORITHM.
	...
	public static byte [] computeBlockDigest(byte [] block) {
		try {
			return computeBlockDigest(NDNDigestHelper.DEFAULT_DIGEST_ALGORITHM, block);
		} catch (NoSuchAlgorithmException e) {
			// DKS --big configuration problem
			Log.warning("Fatal Error: cannot find default algorithm " + NDNDigestHelper.DEFAULT_DIGEST_ALGORITHM);
			throw new RuntimeException("Error: can't find default algorithm " + NDNDigestHelper.DEFAULT_DIGEST_ALGORITHM + "!  " + e.toString());
	...
	public static byte [] computeNodeDigest(String algorithm, byte [] left, byte [] right) throws NoSuchAlgorithmException {
		return NDNDigestHelper.digest(algorithm, left, right);
	}
	...
	public static byte [] computeNodeDigest(byte [] left, byte [] right) {
		try {
			return computeNodeDigest(NDNDigestHelper.DEFAULT_DIGEST_ALGORITHM, left, right);
		} catch (NoSuchAlgorithmException e) {
			// DKS --big configuration problem
			Log.warning("Fatal Error: cannot find default algorithm " + NDNDigestHelper.DEFAULT_DIGEST_ALGORITHM);
			throw new RuntimeException("Error: can't find default algorithm " + NDNDigestHelper.DEFAULT_DIGEST_ALGORITHM + "!  " + e.toString());
