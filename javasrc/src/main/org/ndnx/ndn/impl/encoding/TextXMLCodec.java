...
public class TextXMLCodec implements XMLCodec {
	 ...
	 * standard format for outputing a quantized NDNTime.
	 * @param dateTime the timestamp to encode
	 * @return the formatted timestamp
	 */
	public static String formatDateTime(NDNTime dateTime) {
		// Handles nanoseconds
	 ...
	 * @return the parsed timestamp as a NDNTime
	 */
	public static NDNTime parseDateTime(String strDateTime) throws ParseException {
		
		// no . but has the Z 
		if (strDateTime.indexOf('.') < 0) {
			Date noNsDate = ((SimpleDateFormat)canonicalWriteDateFormat.clone()).parse(strDateTime);
			return new NDNTime(noNsDate);
		}
		...
		NDNTime ts =  new NDNTime(thisDate);
