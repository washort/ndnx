 ...
 *   NDNNetworkChannel is rewindable, so if we read past the end of the
 ...
 *
 * Notes about resync:
 *   - Is it really needed at all?  For instance ndnd won't output a bad packet.
...
//		long value = peekStartElementAsLong();
		...
		if( _elements_type[_parsingElement] == BinaryXMLCodec.XML_DTAG ) {
			if( _elements_value[_parsingElement] == NDNProtocolDTags.Interest ) {
			...
			if( _elements_value[_parsingElement]  == NDNProtocolDTags.ContentObject ) {
...
//		_buffer.position(offset + read);
	...
	private final int readTypeAndValue(final InputStream istream) throws IOException {
		...
		if( typ == BinaryXMLCodec.XML_BLOB || typ == BinaryXMLCodec.XML_UDATA ) {
			if (val < 0 || val > NDNNetworkManager.MAX_PAYLOAD)
...
//				type, elem.position, elem.value, _bytes.length);
	 ...
	 */
	public NDNTime readDateTime(long startTag) throws ContentDecodingException {
		// +1
		readStartElement(startTag);
		// +2
		byte [] byteTimestamp = readBlob();

		return new NDNTime(byteTimestamp);
	}
...
//		return tagToString(peekStartElementAsLong());
	 ...
	 */
	@Deprecated
	public NDNTime readDateTime(String startTag)
			throws ContentDecodingException {
		throw new ContentDecodingException("Don't use STRING values when decoding WIRE objects!");
	}
