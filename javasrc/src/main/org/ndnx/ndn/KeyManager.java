...
import org.ndnx.ndn.protocol.ContentObject.SimpleVerifier;
import org.ndnx.ndn.protocol.KeyLocator;
import org.ndnx.ndn.protocol.KeyLocator.KeyLocatorType;
import org.ndnx.ndn.protocol.KeyName;
...
public abstract class KeyManager {
	 ...
	 * with a version of the NDN protocol is the calculation of the vestigial content digest component
	 ...
	 * Currently this is done by NDNHandle.
	 */
	 ...
	 * @return content name
	 */
	public ContentName getDefaultKeyName(ContentName keyPrefix, PublisherPublicKeyDigest keyID, NDNTime keyVersion) {
		if (null == keyPrefix) {
	 ...
	 * @return
	 */
	public abstract NDNTime getKeyVersion(PublisherPublicKeyDigest keyID);
	 ...
	 * requests it. 
	 * Implementation Note: This code is used in NDNHandle initialization, and as such it
	 * cannot use a NDNHandle or any of the standard network operations without introducing
	...
	public static PublicKeyObject publishKeyToRepository(
		...
		ContentObject availableContent = 
			NDNReader.isVersionedContentAvailable(keyName, ContentType.KEY, keyDigest.digest(), 
	...
	throws IOException {
		...
		// NDN equivalent of a "self-signed cert". Means that
		...
		NDNTime keyVersion = null; // do we force a version?
		...
		if (null != nameAndVersion.second()) {
			...
			keyVersion = new NDNTime(); // so we can use it in locator
		}
