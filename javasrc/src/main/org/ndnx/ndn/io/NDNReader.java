 ...
 * prefer the higher-level interfaces offered by NDNInputStream and
 * its subclasses, or NDNNetworkObject and its subclasses.
 */
public class NDNReader {
	
	protected NDNHandle _handle;
	
	public NDNReader(NDNHandle handle) throws ConfigurationException, IOException {
		_handle = handle;
		if (null == _handle)
	 ...
	 * into a single buffer. Equivalent to NDNWriter's put. Does not do anything about
	 * versioning.
	 */
	public byte [] getData(ContentName name, PublisherPublicKeyDigest publisher, int timeout) throws IOException {
		
		NDNInputStream inputStream = new NDNInputStream(name, publisher, _handle);
	...
	public byte [] getVersionedData(ContentName name, PublisherPublicKeyDigest publisher, int timeout) throws IOException {
		...
		NDNInputStream inputStream = new NDNVersionedInputStream(name, publisher, _handle);
		inputStream.setTimeout(timeout);
	...
	public static ContentObject isContentInRepository(ContentName contentName, ContentType desiredType, 
		...
		// the same (unversioned) name, the repository may get some of the data from the ndnd
		...
		return NDNReader.isContentInRepository(latestAvailableContent, timeout, handle);
	}
	...
	public static ContentObject isVersionedContentAvailable(ContentName contentName, ContentType desiredType, 
		...
		if (null == retrievedObject) {
			...
			if (null != desiredContentDigest) {
				NDNInputStream inputStream = new NDNInputStream(retrievedObject, null, handle);
				byte [] streamContent = NDNDigestHelper.digest(inputStream);
