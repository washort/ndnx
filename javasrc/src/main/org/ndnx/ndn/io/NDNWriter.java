...
/**
 * Simplest interface to putting data into NDN. Puts buffers of data, optionally
 ...
 * interfaces offered by NDNOutputStream and its subclasses, or 
 * NDNNetworkObject and its subclasses.
 */
public class NDNWriter {
	
	protected NDNSegmenter _segmenter;
	 ...
	 * 		of a name (for example /org/ndnx/test).
	 * @param handle The ndn context it will use to write, if null one will be created with NDNHandle#open().
	 ...
	 */
	public NDNWriter(String namespace, NDNHandle handle) throws MalformedContentNameStringException, IOException {
	 ...
	 * @throws IOException If network initialization fails.
	 */
	public NDNWriter(ContentName namespace, NDNHandle handle) throws IOException {
		_segmenter = new NDNSegmenter(getFlowController(namespace, handle));
	}
	 ...
	 * @throws IOException If network initialization fails.
	 */
	public NDNWriter(NDNHandle handle) throws IOException {
	...
	protected NDNFlowControl getFlowController(ContentName namespace, NDNHandle handle) throws IOException {
		if (null != namespace) {
			return new NDNFlowControl(namespace, handle);
		}
		return new NDNFlowControl(handle);
	}
	 ...
	 */
	protected NDNWriter(NDNFlowControl flowControl) {
		_segmenter = new NDNSegmenter(flowControl);
	}
	 ...
	 * 	method of a NDNFilterListener. Only one responder should write data
	 * 	in response to a given Interest. The Interest should ideally have been
	 * 	received on the same NDNHandle used by this NDNWriter to write data.
	 * @throws SignatureException if there is a problem signing.
	 * @throws IOException if there is a problem writing data.
	 */	
	...
	/**
	 * Method for writers used by NDNFilterListeners to output a block
	 ...
	 * content written to this NDNWriter does match it.
	 * 
	 * @param outstandingInterest An interest received prior to constructing
	 *   this writer, ideally on the same NDNHandle that the stream is using
	 ...
	 */
	protected NDNFlowControl getFlowControl() {
