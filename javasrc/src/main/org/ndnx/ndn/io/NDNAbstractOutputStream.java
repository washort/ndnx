 ...
 */
public abstract class NDNAbstractOutputStream extends OutputStream {

	protected NDNHandle _handle = null;
	protected NDNSegmenter _segmenter = null;
	/** 
	 ...
	 * in a NDNSegmenter) and flow control (embodied in a NDNFlowControl,
	 ...
	 */
	public NDNAbstractOutputStream(ContentName baseName,
								   KeyLocator locator, 
								   PublisherPublicKeyDigest publisher,
								   ContentType type,
								   ContentKeys keys,
								   NDNSegmenter segmenter) {
		super();
		_baseName = baseName;
		_type = type;
		...
		_keys = keys;
		Log.info(Log.FAC_IO, "NDNAbstractOutputStream: {0} blocksize is {1}", baseName, _segmenter.getBlockSize());
	}
	
	/**
	 * Special purpose constructor used in tests.
	 */
	protected NDNAbstractOutputStream() {}	
	
	/**
	 * Override in subclasses that need to do something special with start writes 
	 * (see NDNFlowControl#startWrite(ContentName, Shape)). They should call this
	 * superclass method, though, to initialize keys (may need to move this later).
	 * @throws IOException
	 */
	protected void startWrite() throws IOException {
		if (null == _keys) {
			Log.info(Log.FAC_IO, "NDNAbstractOutputStream: startWrite -- searching for keys.");
			_keys = AccessControlManager.keysForOutput(_baseName, _publisher, getType(), _handle);
		}
	}

	/**
	 * Method for streams used by NDNFilterListeners to output a block
	 ...
	 *   this stream, ideally on the same NDNHandle that the stream is using
	...
	/**
	 * @return The NDNSegmenter responsible for segmenting and signing stream content. 
	 */
	protected NDNSegmenter getSegmenter() {
		return _segmenter;
	}
