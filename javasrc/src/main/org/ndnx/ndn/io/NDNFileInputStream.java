...
/**
 * A NDN input stream that expects content names to be versioned, and streams to have a Header
 * containing file-level metadata about each stream. See NDNVersionedInputStream for
 * a description of versioning behavior, and NDNFileOutputStream for a description of
 ...
 * NDNVersionedInputStream. In fact, a NDNVersionedInputStream can be used
 * to read data read by NDNFileOutputStream (except for the header). Using a
 * NDNFileInputStream to read something not written by a NDNFileOutputStream or one
 ...
 *
 */
public class NDNFileInputStream extends NDNVersionedInputStream implements UpdateListener {
	 ...
	 */
	public NDNFileInputStream(ContentName baseName) throws IOException {
		super(baseName);
	}
	 ...
	 * @throws IOException Not currently thrown, will be thrown when constructors retrieve first block.
	 */
	public NDNFileInputStream(ContentName baseName, NDNHandle handle)
									throws IOException {
	 ...
	 * @throws IOException Not currently thrown, will be thrown when constructors retrieve first block.
	 */
	public NDNFileInputStream(ContentName baseName, PublisherPublicKeyDigest publisher,
	 ...
	 * @throws IOException Not currently thrown, will be thrown when constructors retrieve first block.
	 */
	public NDNFileInputStream(ContentName baseName, Long startingSegmentNumber, NDNHandle handle)
										throws IOException {
	 ...
	 * @throws IOException Not currently thrown, will be thrown when constructors retrieve first block.
	 */
	public NDNFileInputStream(ContentName baseName, Long startingSegmentNumber,
	 ...
	 */
	public NDNFileInputStream(ContentName baseName, Long startingSegmentNumber,
			PublisherPublicKeyDigest publisher, ContentKeys keys, NDNHandle handle)
			throws IOException {
	 ...
	 * @throws IOException
	 */
	public NDNFileInputStream(ContentObject startingSegment, EnumSet<FlagTypes> flags, NDNHandle handle)
			throws IOException {
	 ...
	 * @throws IOException
	 */
	public NDNFileInputStream(ContentObject startingSegment, 
				ContentKeys keys, EnumSet<FlagTypes> flags, NDNHandle handle) throws IOException {
	...
	/**
	 * Once we have retrieved the first segment of this stream using NDNVersionedInputStream#getFirstSegment(),
	...
	public long skip(long n) throws IOException {
		...
		if (_currentSegment == null) {
			...
			throw new IOException("Error getting block "+toGetBlockAndOffset[0]+" in NDNInputStream.skip("+n+")");
		} else {
	...
	public void newVersionAvailable(NDNNetworkObject<?> newVersion, boolean wasSave) {
		if (!headerRequested()) {
			if (Log.isLoggable(Log.FAC_IO, Level.WARNING)) {
				Log.warning(Log.FAC_IO, "NDNFileInputStream: got a notification of a new header version {0} when none requested!", 
		...
		if (MetadataProfile.isHeader(newVersion.getBaseName())) {
			...
			if (null != _oldHeader) {
				if (Log.isLoggable(Log.FAC_IO, Level.FINE)) {
					Log.fine(Log.FAC_IO, "NDNFileInputStream: retrieved new header {0}, canceling request for old one.", 
							newVersion.getVersionedName());
				}
			...
			if (Log.isLoggable(Log.FAC_IO, Level.FINE)) {
				Log.fine(Log.FAC_IO, "NDNFileInputStream: retrieved old header {0}, canceling request for new one.", 
						newVersion.getVersionedName());
			}
	...
	public void close() throws IOException{
		Log.info(Log.FAC_IO, "NDNFileInputStream: close {0}.  closing header objects and will make call to shut down pipelining", _baseName);
