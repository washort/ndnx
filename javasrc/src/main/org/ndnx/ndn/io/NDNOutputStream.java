 ...
 * fixed length (see NDNBlockOutputStream for non fixed-length segments).
 */
public class NDNOutputStream extends NDNAbstractOutputStream {
	...
	/**
	 * Constructor for a simple NDN output stream.
	 ...
	 */
	public NDNOutputStream(ContentName baseName, NDNHandle handle) throws IOException {
	 ...
	 * @throws IOException if stream setup fails
	 */
	public NDNOutputStream(ContentName baseName,
	 ...
	 * @throws IOException if stream setup fails
	 */
	public NDNOutputStream(ContentName baseName, ContentKeys keys, NDNHandle handle) throws IOException {
	...
	public NDNOutputStream(ContentName baseName,
						   ...
						   ContentKeys keys,
						   NDNHandle handle) throws IOException {
		this(baseName, locator, publisher, type, keys, new NDNFlowControl(baseName, handle));
		_FCIsOurs = true;
	}
	...
	public NDNOutputStream(ContentName baseName,
		...
		this(baseName, locator, publisher, type, keys, new NDNSegmenter(flowControl, null));
	}
	 ...
	 * @throws IOException if flow controller setup fails
	 */
	protected NDNOutputStream(ContentName baseName,
							  KeyLocator locator,
							  PublisherPublicKeyDigest publisher,
							  ContentType type,
							  ContentKeys keys,
							  NDNSegmenter segmenter) throws IOException {
	...
	public void close() throws IOException {
		try {
			...
			Log.info(Log.FAC_IO, "NDNOutputStream close: {0}", _baseName);
	...
	protected synchronized void flushToNetwork(boolean flushLastBlock, boolean flushNow) throws InvalidKeyException, SignatureException, NoSuchAlgorithmException, InterruptedException, IOException, InvalidAlgorithmParameterException {
		...
		_baseNameIndex = _segmenter.fragmentedPut(_baseName, _baseNameIndex, _buffers,
                                 ...
                                 (flushLastBlock ? NDNSegmenter.LAST_SEGMENT : null),
		...
		if (Log.isLoggable(Log.FAC_IO, Level.INFO))
			Log.info(Log.FAC_IO, "HEADER: NDNOutputStream: flushToNetwork: new _baseNameIndex {0}", _baseNameIndex);
	}
