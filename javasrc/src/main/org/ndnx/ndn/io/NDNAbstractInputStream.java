...
import org.ndnx.ndn.profiles.security.access.AccessControlManager;
 ...
 * bytes segmented and stored in NDN.
 *
 * @see SegmentationProfile for description of NDN segmentation
 */
public abstract class NDNAbstractInputStream extends InputStream implements NDNContentHandler {
	...
	/**
	 * Set up an input stream to read segmented NDN content under a given name.
	...
	public NDNAbstractInputStream(
		...
		Log.info(Log.FAC_IO, "NDNAbstractInputStream: {0} segment {1}", _baseName, _startingSegmentNumber);
		startPipeline();
	}
	 ...
	 * @throws IOException
	 */
	public NDNAbstractInputStream(ContentObject startingSegment,
			ContentKeys keys,
			EnumSet<FlagTypes> flags,
			NDNHandle handle) throws IOException  {
		super();
	...
	private void startPipeline() {
		synchronized (inOrderSegments) {
			...
			if (this instanceof NDNVersionedInputStream && !VersioningProfile.hasTerminalVersion(_basePipelineName)) {
				...
				try {
					...
					Log.warning(Log.FAC_PIPELINE, "Failed to express interest for pipelining segments in NDNAbstractInputStream:  Interest = {0}", interest.name());
				}
			}
		}
	}
				...
				if (_lastSegmentNumber == -1 || _lastRequestedPipelineSegment < _lastSegmentNumber) {
					...
					try {
						...
						if (Log.isLoggable(Log.FAC_PIPELINE, Level.WARNING))
							Log.warning(Log.FAC_PIPELINE, "failed to express interest for NDNAbstractInputStream pipeline: {0}");
						break;
					}
				} else {
					if (Log.isLoggable(Log.FAC_PIPELINE, Level.INFO))
	...
	private void attemptHoleFilling(long hole) {
		...
		try {
			...
			if (Log.isLoggable(Log.FAC_PIPELINE, Level.WARNING))
				Log.warning(Log.FAC_PIPELINE, "failed to express interest for NDNAbstractInputStream pipeline");
		}
	}
		...
		synchronized(inOrderSegments) {
			...
			if (requestedSegment(segmentNumber)) {
				...
				try {
					...
					if (Log.isLoggable(Log.FAC_PIPELINE, Level.WARNING))
						Log.warning(Log.FAC_PIPELINE, "failed to express interest for NDNAbstractInputStream pipeline: {0}", e.getMessage());
				}
			}
	...
	protected void setFirstSegment(ContentObject newSegment) throws IOException {
		...
		while (newSegment.isType(ContentType.LINK) && (!hasFlag(FlagTypes.DONT_DEREFERENCE))) {
			...
			if (Log.isLoggable(Log.FAC_IO, Level.INFO)) {
				Log.info(Log.FAC_IO, "NDNAbstractInputStream: dereferencing link {0} to {1}, resulting data {2}", theLink.getVersionedName(),
						theLink.link(), ((null == newSegment) ? "null" : newSegment.name()));
				Log.info(Log.FAC_SIGNING, "NDNAbstractInputStream: dereferencing link {0} to {1}, resulting data {2}", theLink.getVersionedName(), theLink.link(), ((null == newSegment) ? "null" : newSegment.name()));
			}
	 ...
	 * (see #NDNAbstractInputStream(ContentName, Long, PublisherPublicKeyDigest, ContentKeys, NDNHandle)).
	 ...
	 * specified by startingSegmentNumber, (see #NDNAbstractInputStream(ContentName, Long, PublisherPublicKeyDigest, ContentKeys, NDNHandle)).
	 ...
	 * 	For NDNAbstractInputStream, assume that desiredName contains the name up to but not including
	...
	public void close() throws IOException {
		Log.info(Log.FAC_IO, "NDNAbstractInputStream: close {0}:  shutting down pipelining", _baseName);
	...
	/**
	 * @return Returns position in byte offset. For NDNAbstractInputStream, provide an inadequate
