...
import org.ndnx.ndn.impl.security.crypto.SignatureLocks;
import org.ndnx.ndn.impl.security.crypto.jce.AESWrapWithPad;
...
/**
 * A representation of wrapped (encrypted) keys for strorage in NDN. These are used
...
public class WrappedKey extends GenericXMLEncodable implements XMLEncodable {
	...
	/**
	 * A NDNNetworkObject wrapper around WrappedKey, used for easily saving and retrieving
	 * versioned WrappedKeys to NDN. A typical pattern for using network objects to save
	 ...
	 */
	public static class WrappedKeyObject extends NDNEncodableObject<WrappedKey> {

		public WrappedKeyObject(ContentName name, WrappedKey data, SaveType saveType, NDNHandle handle) throws IOException {
		...
		public WrappedKeyObject(ContentName name, NDNHandle handle) 
		...
		public WrappedKeyObject(ContentObject firstBlock, NDNHandle handle) 
				throws ContentDecodingException, IOException {
	...
	public static byte [] wrappingKeyIdentifier(Key wrappingKey) {
		return NDNDigestHelper.digest(wrappingKey.getEncoded());
	}
	...
	public void decode(XMLDecoder decoder) throws ContentDecodingException {
		...
		if (decoder.peekStartElement(NDNProtocolDTags.WrappingKeyIdentifier)) {
			_wrappingKeyIdentifier = decoder.readBinaryElement(NDNProtocolDTags.WrappingKeyIdentifier); 
		}
		
		if (decoder.peekStartElement(NDNProtocolDTags.WrappingKeyName)) {
			_wrappingKeyName = new WrappingKeyName();
			_wrappingKeyName.decode(decoder);
		}
		
		if (decoder.peekStartElement(NDNProtocolDTags.WrapAlgorithm)) {
			_wrapAlgorithm = decoder.readUTF8Element(NDNProtocolDTags.WrapAlgorithm); 
		}

		if (decoder.peekStartElement(NDNProtocolDTags.KeyAlgorithm)) {
			_keyAlgorithm = decoder.readUTF8Element(NDNProtocolDTags.KeyAlgorithm); 
		}
		...
		if (decoder.peekStartElement(NDNProtocolDTags.EncryptedNonceKey)) {
			_encryptedNonceKey = decoder.readBinaryElement(NDNProtocolDTags.EncryptedNonceKey); 
		}
		
		_encryptedKey = decoder.readBinaryElement(NDNProtocolDTags.EncryptedKey);
	...
	public void encode(XMLEncoder encoder) throws ContentEncodingException {
		...
		if (null != wrappingKeyIdentifier()) {
			...
			encoder.writeElement(NDNProtocolDTags.WrappingKeyIdentifier, wrappingKeyIdentifier());
		}
		...
		if (null != wrapAlgorithm()) {
			...
			encoder.writeElement(NDNProtocolDTags.WrapAlgorithm, wrapAlgorithm());
		}
		
		if (null != keyAlgorithm()) {
			//String keyOID = OIDLookup.getCipherOID(keyAlgorithm());
			encoder.writeElement(NDNProtocolDTags.KeyAlgorithm, keyAlgorithm());
		}		

		if (null != label()) {
			encoder.writeElement(NDNProtocolDTags.Label, label());
		}

		if (null != encryptedNonceKey()) {
			encoder.writeElement(NDNProtocolDTags.EncryptedNonceKey, encryptedNonceKey());
		}

		encoder.writeElement(NDNProtocolDTags.EncryptedKey, encryptedKey());
	...
	public long getElementLabel() { 
		return NDNProtocolDTags.WrappedKey;
	}
