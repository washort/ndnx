...
public class LinkAuthenticator extends GenericXMLEncodable implements XMLEncodable, Comparable<LinkAuthenticator> {
    ...
    protected NDNTime		_timestamp = null;
	...
	public NDNTime timestamp() {
		return _timestamp;
	}
	public void timestamp(NDNTime timestamp) {
	...
	public void decode(XMLDecoder decoder) throws ContentDecodingException {
		...
		if (decoder.peekStartElement(NDNProtocolDTags.NameComponentCount)) {
			_nameComponentCount = decoder.readIntegerElement(NDNProtocolDTags.NameComponentCount);
		}
		...
		if (decoder.peekStartElement(NDNProtocolDTags.Type)) {
			String strType = decoder.readUTF8Element(NDNProtocolDTags.Type);
		...
		if (decoder.peekStartElement(NDNProtocolDTags.ContentDigest)) {
	...
	public void encode(XMLEncoder encoder) throws ContentEncodingException {
		...
		if (!emptyNameComponentCount()) {
			encoder.writeElement(NDNProtocolDTags.NameComponentCount, nameComponentCount());
		}

		if (!emptyTimestamp()) {
			encoder.writeDateTime(NDNProtocolDTags.Timestamp, timestamp());
		}
		
		if (!emptyContentType()) {
			encoder.writeElement(NDNProtocolDTags.Type, SignedInfo.typeToName(type()));
		}
	...
	@Override
	public long getElementLabel() { return NDNProtocolDTags.LinkAuthenticator; }
