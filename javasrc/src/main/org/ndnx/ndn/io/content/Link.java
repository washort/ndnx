...
/**
 * Represents a secure, authenticatable link from one part of the NDN namespace to another.
 * 
 * NDN links are very flexible and can be used to represent a wide variety of application-level
 ...
 * were my children".
 * 
 * NDN links have authentication information associated with them, and can be made highly secure --
 ...
 * 
 * Links also play an important role in making up Collections, the NDN notion of a container full
 * of objects or names.
 */
public class Link extends GenericXMLEncodable implements XMLEncodable, Cloneable {
	
	/**
	 * A NDNNetworkObject wrapper around Link, used for easily saving and retrieving
	 * versioned Links to NDN. A typical pattern for using network objects to save
	 ...
	 */
	public static class LinkObject extends NDNEncodableObject<Link> {
		
		public LinkObject(ContentName name, Link data, SaveType saveType, NDNHandle handle) throws IOException {
		...
		public LinkObject(ContentName name, NDNHandle handle) 
				throws ContentDecodingException, IOException {
			super(Link.class, true, name, (PublisherPublicKeyDigest)null, handle);
		}
		
		public LinkObject(ContentName name, PublisherPublicKeyDigest publisher, NDNHandle handle) 
				throws ContentDecodingException, IOException {
			super(Link.class, true, name, publisher, handle);
		}

		public LinkObject(ContentObject firstBlock, NDNHandle handle) 
				throws ContentDecodingException, IOException {
		...
		public LinkObject(ContentObject firstBlock, NDNFlowControl flowControl)
				throws ContentDecodingException, IOException {
		...
		public LinkObject(NDNEncodableObject<? extends Link> other) {
	 ...
	 * @throws IOException 
	 */
	public ContentObject dereference(long timeout, NDNHandle handle) throws IOException {
	...
	public void decode(XMLDecoder decoder) throws ContentDecodingException {
		...
		if (decoder.peekStartElement(NDNProtocolDTags.Label)) {
			_targetLabel = decoder.readUTF8Element(NDNProtocolDTags.Label); 
		}

		if (decoder.peekStartElement(NDNProtocolDTags.LinkAuthenticator)) {
	...
	public void encode(XMLEncoder encoder) throws ContentEncodingException {
		...
		if (null != targetLabel()) {
			encoder.writeElement(NDNProtocolDTags.Label, targetLabel());
		}
	...
	@Override
	public long getElementLabel() { return NDNProtocolDTags.Link; }
