...
import org.ndnx.ndn.impl.NDNFlowControl.Shape;
...
import org.ndnx.ndn.io.LinkCycleException;
import org.ndnx.ndn.io.NoMatchingContentFoundException;
...
/**
 * Extends a NetworkObject to add specifics for using a NDN-based backing store. Each time
 * the object is saved creates a new NDN version. Readers can open a specific version or
 ...
 *
 * These objects attempt to maintain a NDN copy of the current state of their data. In descriptions
 * below, an object that is "dirty" is one whose data has been modified locally, but not yet
 * saved to the network.
 *
 * While NDNNetworkObject could be used directly, it almost never is; it is usually
 ...
 *
 * TODO: Note that the NDNNetworkObject class hierarchy currently has a plethora of constructors.
 ...
 */
public abstract class NDNNetworkObject<E> extends NetworkObject<E> implements NDNContentHandler {
	 ...
	 * create.
	 */
	protected NDNFlowControl _flowControl;
	 ...
	 * @param handle NDNHandle to use for network operations. If null, a new one is created using NDNHandle#open().
	 * @throws IOException If there is an error setting up network backing store.
	 */
	public NDNNetworkObject(Class<E> type, boolean contentIsMutable,
							ContentName name, E data, SaveType saveType, NDNHandle handle) throws IOException {
	 ...
	 *   NDNFlowControl#startWrite(ContentName, Shape) on each save. Calls to
	 ...
	 * @throws IOException If there is an error setting up network backing store.
	 */
	protected NDNNetworkObject(Class<E> type, boolean contentIsMutable,
	 ...
	 * @param flowControl Flow controller to use. A single flow controller object
	 *   is used for all this instance's writes, we use underlying streams to call
	 *   NDNFlowControl#startWrite(ContentName, Shape) on each save.
	 * @throws ContentDecodingException if there is a problem decoding the object.
	 * @throws IOException if there is an error setting up network backing store.
	 */
	protected NDNNetworkObject(Class<E> type, boolean contentIsMutable,
	 ...
	 */
	public NDNNetworkObject(Class<E> type, boolean contentIsMutable,
							ContentObject firstSegment, NDNHandle handle)
			throws ContentDecodingException, IOException {
		super(type, contentIsMutable);
		if (null == handle) {
			try {
	 ...
	 */
	protected NDNNetworkObject(Class<E> type, NDNNetworkObject<? extends E> other) {
		super(type, other);
	...
	protected synchronized void createFlowController() throws IOException {
		if (null == _flowControl) {
			...
			switch (_saveType) {
			case RAW:
				_flowControl = new NDNFlowControl(_handle);
				break;
			case REPOSITORY:
	 ...
	 */
	public NDNFlowControl getFlowControl() {
		return _flowControl;
	}
	 ...
	 */
	protected void setInputStreamProperties(NDNInputStream inputStream) {
	 ...
	 * happy with the latest available version available in your local ndnd cache; or you really
	 ...
	 * i.e. if there are two versions say in your local ndnd cache (or repo with nothing in the ndnd
	 ...
	 * version in the repo than one in the ndnd cache. Use this if you know there is only one version
	...
	public boolean update(ContentName name, PublisherPublicKeyDigest publisher) throws ContentDecodingException, IOException {
		...
		NDNVersionedInputStream is = new NDNVersionedInputStream(name, publisher, _handle);
		return update(is);
	}
	...
	public boolean update(ContentObject object) throws ContentDecodingException, IOException {
		NDNInputStream is = new NDNInputStream(object, getInputStreamFlags(), _handle);
	...
	/**
	 * Updates the object from a NDNInputStream or one of its subclasses. Used predominantly
	 ...
	 * @throws IOException if there is an error setting up network backing store.
	 */
	public synchronized boolean update(NDNInputStream inputStream) throws ContentDecodingException, IOException {
	...
	/**
	 * Method for NDNFilterListeners to save an object in response to an Interest
	 ...
	 * been received on the NDNHandle the object is using for output. If the object
	 ...
	 * @throws IOException if there is an error reading the content from the network
	 */
	public boolean save(NDNTime version) throws ContentEncodingException, IOException {
	 ...
	 * to writing its blocks.)
	 */
	public boolean save(NDNTime version, Interest outstandingInterest)
	 ...
	 * @throws IOException if there is an error reading the content from the network
	 */
	protected synchronized boolean saveInternal(NDNTime version, boolean gone, Interest outstandingInterest)
				throws ContentEncodingException, IOException {
		...
		ContentName name = new ContentName(_baseName, version == null ? NDNTime.now() : version);
		...
		if (!gone) {
			// NDNVersionedOutputStream will version an unversioned name.
			...
			// NDNFlowServer which requires that the flow controller retain its objects after writing
			// them. A standard FC would cause the objects to be lost.
			NDNVersionedOutputStream cos = new NDNVersionedOutputStream(name, _keyLocator, _publisher, contentType(), _keys, _flowControl);
		...
		// ourselves registered with ndnd. That could cause interests to be unnecessarily or incorrectly
	 ...
	 * @throws IOException if there is an error reading the content from the network
	 */
	public synchronized boolean save(NDNTime version, E data) throws ContentEncodingException, IOException {
		setData(data);
		return save(version);
	}
	 ...
	 */
	@Deprecated
	public synchronized boolean saveToRepository(NDNTime version) throws ContentEncodingException, IOException {
		if (null == _baseName) {
			throw new IllegalStateException("Cannot save an object without giving it a name!");
		}
	...
	public boolean saveToRepository() throws ContentEncodingException, IOException {
		return saveToRepository((NDNTime)null);
	}
	 ...
	 */
	@Deprecated
	public synchronized boolean saveToRepository(NDNTime version, E data) throws ContentEncodingException, IOException {
		setData(data);
	...
	/**
	 * For use by NDNFilterListeners, saves a GONE object and emits an initial
	...
	public synchronized NDNTime getVersion() throws IOException {
		if (isSaved())
	...
	protected class Updater {
		...
		protected NDNContentHandler _handler = null;
		protected boolean _isRunning = false;

		protected Updater(NDNContentHandler handler) {
			_handler = handler;
		}
								...
								if (VersioningProfile.isVersionedFirstSegment(_currentInterest.name(), co, null)) {
									...
									if (!_verifier.verify(co)) {
										if (Log.isLoggable(Log.FAC_SIGNING, Level.WARNING)) {
											Log.warning(Log.FAC_SIGNING, "NDNNetworkObject: content object received from background update did not verify! Ignoring object: {0}", co.fullName());
										}
										hasNewVersion = false;
	 ...
	 */
	protected void doSave(NDNTime version, boolean gone, Interest outstandingInterest, boolean doClose) {
	...
	protected class BackgroundSaver implements Runnable {
		...
		protected NDNTime _version = null;
		protected Interest _outstandingInterest = null;
		protected boolean _doClose = false;

		public BackgroundSaver(NDNTime version, boolean gone, Interest outstandingInterest, boolean doClose) {
			_version = version;
			_gone = gone;
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		NDNNetworkObject<E> other = (NDNNetworkObject<E>) obj;
		if (_baseName == null) {
