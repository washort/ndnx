 ...
 * argument, or %C1 for an argument which is NDNB-encoded data. (Multiple
 * binary arguments can be provided using NDNB-encoding.)
 * 
 ...
 * comes last. 
 * Namespaces containing only capital letters are reserved for NDN itself,
 * and are listed here.
 * 
 ...
 * 
 * %C1.org.ndnx.frobnicate~1~37
 * would be a command in the namespace "org.ndnx", where the command is "frobnicate",
 ...
 * 
 * A namespace org.ndnx.foo could have an operation bar, that took a single ndnb-encoded argument:
 * %C1.org.ndnx.foo.bar%C1<argument>
 * 
 * or 2 UTF-8 arguments and a binary argument
 * %C1.org.ndnx.foo.bar~arg1~arg1%00<binary argument>
 * 
...
public class CommandMarker implements ContentName.ComponentProvider {
	 ...
	 * Reserved bytes.
	 */
	public static byte[] NDN_reserved_markers = { (byte)0xC0, (byte)0xC1, (byte)0xF5, 
	...
	public static final byte NDNB_ARGUMENT_SEPARATOR = (byte)0xC1;
	...
	/**
	 * Helper method if you just need to add data.
	 * @param applicationData -- ndnb encoded
	 * @return
	 */
	public byte [] addNDNBEncodedData(byte [] applicationData) {
		return addArgumentsAndData(null, applicationData, NDNB_ARGUMENT_SEPARATOR);
	}
	...
	public static boolean isNDNBApplicationData(byte [] nameComponent) {
		if (!isCommandComponent(nameComponent)) {
			throw new IllegalArgumentException("Not a command marker!");
		}
		...
		return nameComponent[argumentStart] == NDNB_ARGUMENT_SEPARATOR;
	}
