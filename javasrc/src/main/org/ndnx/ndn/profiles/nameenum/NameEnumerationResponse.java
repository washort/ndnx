...
public class NameEnumerationResponse {
	...
	public static class NameEnumerationResponseMessage extends Collection {
		...
		public static class NameEnumerationResponseMessageObject extends NDNEncodableObject<NameEnumerationResponseMessage> {
			
			public NameEnumerationResponseMessageObject(ContentName name, NameEnumerationResponseMessage data, NDNHandle handle) throws IOException {
				super(NameEnumerationResponseMessage.class, true, name, data, SaveType.RAW, null, null, handle);
			}
			
			public NameEnumerationResponseMessageObject(ContentName name, java.util.Collection<Link> contents, NDNHandle handle) throws IOException {
				this(name, new NameEnumerationResponseMessage(contents), handle);
			}
			
			public NameEnumerationResponseMessageObject(ContentName name, Link [] contents, NDNHandle handle) throws IOException {
				this(name, new NameEnumerationResponseMessage(contents), handle);			
			}
			...
			public NameEnumerationResponseMessageObject(ContentName name, NDNHandle handle) 
			...
			public NameEnumerationResponseMessageObject(ContentName name, PublisherPublicKeyDigest publisher, NDNHandle handle) 
					throws ContentDecodingException, IOException {
				super(NameEnumerationResponseMessage.class, true, name, publisher, handle);
				setSaveType(SaveType.RAW);
			}
			
			public NameEnumerationResponseMessageObject(ContentObject firstBlock, NDNHandle handle) 
					throws ContentDecodingException, IOException {
	 ...
	 * @param ts NDNTime is the timestamp used to create the version component
	 *   for the object when it is written out
	 */
	public NameEnumerationResponse(ContentName p, ArrayList<ContentName> n, NDNTime ts) {
		_prefix = p;
		_names = n;
		_version = ts;
	}
	 ...
	 */
	public NameEnumerationResponse(ContentName p, byte [][] names, NDNTime ts) {
		_prefix = p;
		_version = ts;
	 ...
	 * @param ts NDNTime for the ContentObject version
	 * @return void
	 */
	public void setTimestamp(NDNTime ts) {
		_version = ts;
	}
	 ...
	 * 
	 * @return NDNTime for the version component of the object
	 */
	public NDNTime getTimestamp() {
		return _version;
	}
