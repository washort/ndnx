...
import org.ndnx.ndn.NDNContentInterest;
import org.ndnx.ndn.NDNHandle;
import org.ndnx.ndn.NDNInterestHandler;
import org.ndnx.ndn.impl.QueuedContentHandler;
 ...
 *
 * @see NDNInterestHandler
 * @see NDNContentHandler
 * @see BasicNameEnumeratorListener
 * @see NameEnumerationResponse
 *
 */

public class NDNNameEnumerator implements NDNInterestHandler, NDNContentHandler {
	...
	/**
	 * A supporting class for NDNNameEnumerator.  NERequest objects hold registered prefixes and
	...
	/**
	 * A supporting class for NDNNameEnumerator.  NEResponse objects hold ContentName responses
	 ...
	 */
	protected class NEHandler extends QueuedContentHandler<NDNContentInterest> {
		protected NDNHandle _handle;
		protected NDNContentHandler _handler;

		protected NEHandler(NDNHandle handle, NDNContentHandler handler) {
			_handle = handle;
			_handler = handler;
		}

		@Override
		public void process(NDNContentInterest ci) {
			...
			if (c != null) {
				...
				if (responseName==null ) {
					...
					Log.warning(Log.FAC_SEARCH, "NDNNameEnumerator received a response without a responseID: {0} matching interest {1}", c.name(), interest.name());
				} else {
				...
				try {
					...
					Log.warning(Log.FAC_SEARCH, "Error parsing Collection from ContentObject in NDNNameEnumerator");
					Log.warningStackTrace(Log.FAC_SEARCH, e);
				} catch(IOException e) {
					Log.warning(Log.FAC_SEARCH, "error getting CollectionObject from ContentObject in NDNNameEnumerator.handleContent");
					Log.warningStackTrace(Log.FAC_SEARCH, e);
				}
			}
		}
	...
	/**
	 * NDNNameEnumerator constructor.  Creates a NDNNameEnumerator, sets the NDNHandle,
	 * registers the callback and registers a prefix for enumeration.
	 *
	 * @param prefix ContentName to enumerate names under
	 * @param handle NDNHandle for sending and receiving collection objects during enumeration.
	 * @param c BasicNameEnumeratorListener callback to receive enumeration responses.
	 */

	public NDNNameEnumerator(ContentName prefix, NDNHandle handle, BasicNameEnumeratorListener c) throws IOException {
		_handle = handle;
	...
	/**
	 * NDNNameEnumerator constructor.  Creates a NDNNameEnumerator, sets the NDNHandle, and
	...
	public NDNNameEnumerator(NDNHandle handle, BasicNameEnumeratorListener c) {
		_handle = handle;
		_neHandler = new NEHandler(handle, this);
		callback = c;
	}
	...
	public Interest handleContent(ContentObject c, Interest interest) {
		...
		_neHandler.add(new NDNContentInterest(c, interest));
		return null;
	}
