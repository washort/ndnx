...
import org.ndnx.ndn.protocol.ContentNameProvider;
 ...
 * Currently implemented as a wrapper around NDNNameEnumerator, will likely directly aggregate
 * name enumeration responses in the future.
 *
 * @see NDNNameEnumerator
...
public class EnumeratedNameList implements BasicNameEnumeratorListener, ContentNameProvider {
	...
	protected NDNNameEnumerator _enumerator;
	...
	protected NDNTime _lastUpdate = null;
	 ...
	 * the children of the specified prefix.  The new EnumeratedNameList will use the NDNHandle passed
	 * in to the constructor, or create a new one using NDNHandle#open() if it is null.
	 *
	 * @param  namePrefix the ContentName whose children we wish to list.
	 * @param  handle the NDNHandle object for sending interests and receiving content object responses.
	 */
	public EnumeratedNameList(ContentName namePrefix, NDNHandle handle) throws IOException {
		this(namePrefix, true, handle);
	}

	public EnumeratedNameList(ContentName namePrefix, boolean startEnumerating, NDNHandle handle) throws IOException {
		if (null == namePrefix) {
		...
		if (startEnumerating) {
			_enumerating = true;
			_hasEnumerated = true;
			_enumerator = new NDNNameEnumerator(namePrefix, handle, this);
		} else {
			_enumerator = new NDNNameEnumerator(handle, this);
		}
	}
	...
	public boolean waitForNewChildren(boolean threadPoolContext, long timeout) {
		...
		synchronized(_childLock) {
			NDNTime lastUpdate = _lastUpdate;
		...
		while (it.hasNext()) {
			...
			if (VersioningProfile.isVersionComponent(theName.component(0))) {
				if (null == latestName) {
					...
					NDNTime thisTimestamp = VersioningProfile.getVersionComponentAsTimestamp(theName.component(0));
	...
	/**
	 * Handle responses from NDNNameEnumerator that give us a list of single-component child
	...
	/**
	 * Returns the latest version available under this prefix as a NDNTime object.
	 *
	 * @return NDNTime Latest child version as NDNTime
	 */
	public NDNTime getLatestVersionChildTime() {
	 ...
	 * @param handle NDNHandle to use for enumeration
	 * @return ContentName The name supplied to the call with the latest version added.
	 * @throws IOException
	 */
	public static ContentName getLatestVersionName(ContentName name, NDNHandle handle) throws IOException {
	 ...
	 * @param handle NDNHandle for sending and receiving interests and content objects.
	 *
	 ...
	 * @throws IOException
	 */
	public static EnumeratedNameList exists(ContentName childName, ContentName prefixKnownToExist, NDNHandle handle) throws IOException {
		if (Log.isLoggable(Log.FAC_SEARCH, Level.INFO))
	 ...
	 * @see ContentName#builder(org.ndnx.ndn.protocol.ContentName.StringParser, Object[])
	 */
