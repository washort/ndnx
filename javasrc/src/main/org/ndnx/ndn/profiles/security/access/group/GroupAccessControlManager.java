...
import org.ndnx.ndn.impl.support.ByteArrayCompare;
import org.ndnx.ndn.impl.support.DataUtils;
import org.ndnx.ndn.impl.support.Log;
import static org.ndnx.ndn.impl.support.Log.*;
import static org.ndnx.ndn.io.content.KeyDirectory.PREVIOUS_KEY;
...
import org.ndnx.ndn.profiles.namespace.NamespaceProfile;
import org.ndnx.ndn.profiles.namespace.ParameterizedName;
import org.ndnx.ndn.profiles.search.LatestVersionPathfinder;
...
import org.ndnx.ndn.profiles.security.access.AccessControlPolicyMarker;
import org.ndnx.ndn.profiles.security.access.AccessControlProfile;
import org.ndnx.ndn.profiles.security.access.AccessDeniedException;
import org.ndnx.ndn.profiles.security.access.AccessControlPolicyMarker.AccessControlPolicyMarkerObject;
import org.ndnx.ndn.profiles.security.access.group.ACL.ACLObject;
import org.ndnx.ndn.profiles.security.access.group.ACL.ACLOperation;
import org.ndnx.ndn.profiles.security.access.group.GroupAccessControlProfile.PrincipalInfo;
 ...
 * see the NDNx Access Control Specification. Management of Groups and Group members is
...
public class GroupAccessControlManager extends AccessControlManager {
	 ...
	 */
	public static final String PROFILE_NAME_STRING = "/ndnx.org/ndn/profiles/security/access/group/GroupAccessControlProfile";
	...
	public GroupAccessControlManager(ContentName namespace, NDNHandle handle) throws IOException {
	...
	public GroupAccessControlManager(ContentName namespace, ContentName groupStorage, 
									 ContentName userStorage, NDNHandle handle) 
			throws IOException {		
	...
	public static AccessControlManager create(ContentName name, ContentName profileName, 
			...
			KeyValueSet parameters, SaveType saveType, NDNHandle handle) throws IOException, InvalidKeyException {
	...
	private void initialize(ContentName namespace, ContentName[] groupStorage, 
		...
		if (null == handle) handle = NDNHandle.getHandle();
	...
	@Override
	public boolean initialize(AccessControlPolicyMarkerObject policyInformation, NDNHandle handle) 
				throws IOException {
		if (null == handle) {
	...
	/**
	 * Publish my identity (i.e. my public key) under a specified NDN name
	 ...
	 */
	public boolean isProtectedContent(ContentName name, PublisherPublicKeyDigest publisher, ContentType contentType, NDNHandle handle) {
		if (isGroupName(name)) {
