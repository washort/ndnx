...
import static org.ndnx.ndn.io.content.KeyDirectory.GROUP_PRIVATE_KEY;
import static org.ndnx.ndn.io.content.KeyDirectory.GROUP_PUBLIC_KEY;
 ...
 * For descriptions of data, and how this access control system functions, see the separate NDNx Access
 ...
 */
public class GroupAccessControlProfile extends AccessControlProfile implements NDNProfile {
	...
	/**
	 * This class records information about a NDN principal.
	...
	public static class PrincipalInfo {
		...
		private NDNTime _versionTimestamp;
		...
		public PrincipalInfo(byte [] principalInfoNameComponent) {
			...
			try {
				...
				pos += NDNProfile.COMPONENT_SEPARATOR.length;
				...
				int fnLength = 0;
				while (principalInfoNameComponent[pos + fnLength] != NDNProfile.COMPONENT_SEPARATOR[0]) fnLength++;
				...
				_versionTimestamp = new NDNTime(timestampBytes);
			} catch (Exception e) {
		...
		public NDNTime versionTimestamp() { return _versionTimestamp; }
		...
		public static byte [] contentPrefixToDistinguishingHash(ContentName name) {
			...
			fullDigest = NDNDigestHelper.digest(encoded);
