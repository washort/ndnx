...
/**
 * Represent the version used in a NDNx name.
 * 
 * The core of this is based on a NDNTime, but NDNTime has two different
...
public class VersionNumber implements Comparable<VersionNumber>, ContentName.ComponentProvider {
	 ...
	 * @param version
	 */
	public VersionNumber(NDNTime version) {
		_version = new NDNTime(version);
		_versionComponent = VersioningProfile.timeToVersionComponent(_version);
		_binaryTime = _version.toBinaryTimeAsLong();
	}
	 ...
	 * (as per System.currentTimeMillis or NDNTime.getTime).
	 * @param msecSinceEpoch
	 */
	public VersionNumber(long msecSinceEpoch) {
		_version = new NDNTime(msecSinceEpoch);
		_versionComponent = VersioningProfile.timeToVersionComponent(_version);
		_binaryTime = _version.toBinaryTimeAsLong();
	}
	...
	/**
	 * Returns a new NDNTime object representing the version
	 */
	public NDNTime getAsTime() {
		return new NDNTime(_version);
	}
...
//	}
	...
	public VersionNumber addAndReturn(long count) {
		...
		binaryTime += count;
		return new VersionNumber(NDNTime.fromBinaryTimeAsLong(binaryTime));
	}
	...
	public static VersionNumber now() {
		return new VersionNumber(NDNTime.now());
	}
	 ...
	 * @return true if this version is strictly less than #time
	 */
	public boolean before(NDNTime time) {
	 ...
	 * @return true if this version is strictly greater than #time
	 */
	public boolean after(NDNTime time) {
		return _version.compareTo(time) > 0;
	}
	
	// =========================================
	// Internal implementation
	protected final NDNTime _version;
