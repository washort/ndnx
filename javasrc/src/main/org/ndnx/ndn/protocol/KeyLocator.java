 ...
 * containing the key, or a NDN name pointing to a location where the key can be found.
 */
    ...
    */
	...
	public void decode(XMLDecoder decoder) throws ContentDecodingException {
		...
		if (decoder.peekStartElement(NDNProtocolDTags.Key)) {
			try {
				byte [] encodedKey = decoder.readBinaryElement(NDNProtocolDTags.Key);
			...
			try {
				byte [] encodedCert = decoder.readBinaryElement(NDNProtocolDTags.Certificate);
	...
	public void encode(XMLEncoder encoder) throws ContentEncodingException {
		...
		if (type() == KeyLocatorType.KEY) {
			encoder.writeElement(NDNProtocolDTags.Key, key().getEncoded());
		} else if (type() == KeyLocatorType.CERTIFICATE) {
			try {
				encoder.writeElement(NDNProtocolDTags.Certificate, certificate().getEncoded());
			} catch (CertificateEncodingException e) {
	...
	@Override
	public long getElementLabel() { return NDNProtocolDTags.KeyLocator; }
