...
public class PublisherID extends GenericXMLEncodable implements XMLEncodable, Comparable<PublisherID>, Serializable {
    ...
    public enum PublisherType {
    	KEY 		(NDNProtocolDTags.PublisherPublicKeyDigest), 
    	CERTIFICATE (NDNProtocolDTags.PublisherCertificateDigest), 
    	ISSUER_KEY	(NDNProtocolDTags.PublisherIssuerKeyDigest), 
    	ISSUER_CERTIFICATE	(NDNProtocolDTags.PublisherIssuerCertificateDigest);
    	...
    	public static PublisherType tagValToType(long tagVal) {
        	...
        	if (tagVal == NDNProtocolDTags.PublisherCertificateDigest) {
        		return CERTIFICATE;
        	}
        	if (tagVal == NDNProtocolDTags.PublisherIssuerKeyDigest) {
        		return ISSUER_KEY;
        	}
        	if (tagVal == NDNProtocolDTags.PublisherIssuerCertificateDigest) {
    ...
    */
    ...
    public static byte [] generateCertificateDigest(String digestAlg, X509Certificate cert) 
        ...
        try {
            ...
            id = NDNDigestHelper.digest(digestAlg, encoding);
	...
	public String toString() {
		...
		return type().name() + ":" + NDNDigestHelper.printBytes(id(), 32);
	}
}
