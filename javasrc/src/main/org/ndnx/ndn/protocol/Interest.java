...
/**
 * Represents a NDN Interest packet, and performs all the allowed specializations
...
public class Interest extends GenericXMLEncodable implements XMLEncodable, Comparable<Interest>, Cloneable, ContentNameProvider {
	...
	/**
	 * XXX - This isn't user settable and is only useful for ndnd internal functionality. 
	...
	public void decode(XMLDecoder decoder) throws ContentDecodingException {
		...
		if (decoder.peekStartElement(NDNProtocolDTags.MinSuffixComponents)) {
			_minSuffixComponents = decoder.readIntegerElement(NDNProtocolDTags.MinSuffixComponents);
		}
		
		if (decoder.peekStartElement(NDNProtocolDTags.MaxSuffixComponents)) {
			_maxSuffixComponents = decoder.readIntegerElement(NDNProtocolDTags.MaxSuffixComponents);
		}
		...
		if (decoder.peekStartElement(NDNProtocolDTags.ChildSelector)) {
			_childSelector = decoder.readIntegerElement(NDNProtocolDTags.ChildSelector);
		}
		
		if (decoder.peekStartElement(NDNProtocolDTags.AnswerOriginKind)) {
			// call setter to handle defaulting
			_answerOriginKind = decoder.readIntegerElement(NDNProtocolDTags.AnswerOriginKind);
		}
		
		if (decoder.peekStartElement(NDNProtocolDTags.Scope)) {
			_scope = decoder.readIntegerElement(NDNProtocolDTags.Scope);
		}

		if (decoder.peekStartElement(NDNProtocolDTags.InterestLifetime)) {
			_interestLifetime = decoder.readBinaryElement(NDNProtocolDTags.InterestLifetime);
		}
		
		if (decoder.peekStartElement(NDNProtocolDTags.Nonce)) {
			_nonce = decoder.readBinaryElement(NDNProtocolDTags.Nonce);
		}
	...
	public void encode(XMLEncoder encoder) throws ContentEncodingException {
		...
		if (null != minSuffixComponents()) 
			encoder.writeElement(NDNProtocolDTags.MinSuffixComponents, minSuffixComponents());	

		if (null != maxSuffixComponents()) 
			encoder.writeElement(NDNProtocolDTags.MaxSuffixComponents, maxSuffixComponents());
		...
		if (null != childSelector()) 
			encoder.writeElement(NDNProtocolDTags.ChildSelector, childSelector());

		if (DEFAULT_ANSWER_ORIGIN_KIND != answerOriginKind()) 
			encoder.writeElement(NDNProtocolDTags.AnswerOriginKind, answerOriginKind());

		if (null != scope()) 
			encoder.writeElement(NDNProtocolDTags.Scope, scope());
		
		if (null != nonce())
			encoder.writeElement(NDNProtocolDTags.Nonce, nonce());
		
		encoder.writeEndElement();   		
	}
	
	@Override
	public long getElementLabel() { return NDNProtocolDTags.Interest; }
