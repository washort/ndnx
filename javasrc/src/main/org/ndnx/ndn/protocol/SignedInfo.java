 ...
 * <br>
 * a) the stuff "about" a piece of NDN data that it is legal to expect routers to understand and
 ...
 *    look for /ndn/foo/&#42;/bar for reasons of routing efficiency, except through a slower
...
public class SignedInfo extends GenericXMLEncodable implements XMLEncodable {
    ...
    public SignedInfo(
    	...
    	if (null == timestamp) {
    		this._timestamp = NDNTime.now(); // msec only
    	} else {
	 ...
	 */
	public final NDNTime getTimestamp() { return _timestamp; }
	...
	public void decode(XMLDecoder decoder) throws ContentDecodingException {
		...
		if (decoder.peekStartElement(NDNProtocolDTags.Timestamp)) {
			_timestamp = decoder.readDateTime(NDNProtocolDTags.Timestamp);
		}

		if (decoder.peekStartElement(NDNProtocolDTags.Type)) {
			byte [] binType = decoder.readBinaryElement(NDNProtocolDTags.Type);
		...
		if (decoder.peekStartElement(NDNProtocolDTags.FreshnessSeconds)) {
			_freshnessSeconds = decoder.readIntegerElement(NDNProtocolDTags.FreshnessSeconds);
		}
		
		if (decoder.peekStartElement(NDNProtocolDTags.FinalBlockID)) {
			_finalBlockID = decoder.readBinaryElement(NDNProtocolDTags.FinalBlockID);
		}
		...
		if (decoder.peekStartElement(NDNProtocolDTags.ExtOpt)) {
			_extOpt = decoder.readBinaryElement(NDNProtocolDTags.ExtOpt);
		}
	...
	public void encode(XMLEncoder encoder) throws ContentEncodingException {
		...
		if (!emptyTimestamp()) {
			encoder.writeDateTime(NDNProtocolDTags.Timestamp, getTimestamp());
		}
		
		if (!defaultContentType()) {
			// DATA is default, element is optional, so omit if DATA
			encoder.writeElement(NDNProtocolDTags.Type, getTypeValue());
		}
		
		if (!emptyFreshnessSeconds()) {
			encoder.writeElement(NDNProtocolDTags.FreshnessSeconds, getFreshnessSeconds());
		}

		if (!emptyFinalBlockID()) {
			encoder.writeElement(NDNProtocolDTags.FinalBlockID, getFinalBlockID());
		}
		...
		if (!emptyExtOpt()) {
			encoder.writeElement(NDNProtocolDTags.ExtOpt, getExtOpt());
		}
	...
	@Override
	public long getElementLabel() { return NDNProtocolDTags.SignedInfo; }
