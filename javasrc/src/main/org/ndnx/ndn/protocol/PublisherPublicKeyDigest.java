...
import org.ndnx.ndn.protocol.ContentName.ComponentProvider;


/**
 * Wrapper class around the digest of public keys used as a publisher identifier in ContentObjects.
 * The digest algorithm used to compute publisherKeyIDs is specified by the NDNx protocol, and changes
 * rarely with the NDNx protocol version (for example, now it is SHA-256, it will shift to SHA-3).
 ...
 * this one of the small number of fixed-algorithm components in NDNx, to vastly simplify what is
...
public class PublisherPublicKeyDigest extends GenericXMLEncodable 
	...
	public void decode(XMLDecoder decoder) throws ContentDecodingException {		
		...
		if (_publisherPublicKeyDigest.length != PublisherID.PUBLISHER_ID_LEN) {
			if (Log.isLoggable(Level.WARNING)) {
				Log.warning("Wrong length for PublisherPublicKeyDigest: {0}", NDNDigestHelper.printBytes(_publisherPublicKeyDigest, 32));
			}
	...
	@Override
	public long getElementLabel() { return NDNProtocolDTags.PublisherPublicKeyDigest; }
