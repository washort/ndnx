...
import org.ndnx.ndn.impl.security.crypto.NDNSignatureHelper;
import org.ndnx.ndn.impl.support.DataUtils;
import org.ndnx.ndn.impl.support.Log;
import org.ndnx.ndn.io.NullOutputStream;
...
public class ContentObject extends GenericXMLEncodable implements XMLEncodable, Comparable<ContentObject>, ContentNameProvider {
	...
	public ContentObject(String digestAlgorithm, // prefer OID
		...
		if ((null != signature) && Log.isLoggable(Log.FAC_SIGNING, Level.FINEST)) {
			try {
				byte [] digest = NDNDigestHelper.digest(this.encode());
				byte [] tbsdigest = NDNDigestHelper.digest(prepareContent());
	...
	public ContentObject(
			...
			SignedInfo signedInfo,
			InputStream contentStream, int length) throws IOException {
		this(NDNDigestHelper.DEFAULT_DIGEST_ALGORITHM, name, signedInfo, contentStream, length);
	}
	
	public ContentObject(ContentName name, SignedInfo signedInfo, byte [] content,
			Signature signature) {
		this(NDNDigestHelper.DEFAULT_DIGEST_ALGORITHM, name, signedInfo, content, signature);
	}

	public ContentObject(ContentName name, SignedInfo signedInfo, 
			byte [] content, int offset, int length,
			Signature signature) {
		this(NDNDigestHelper.DEFAULT_DIGEST_ALGORITHM, name, signedInfo, content, offset, length, signature);
	}
	...
	public void decode(XMLDecoder decoder) throws ContentDecodingException {
		...
		_content = decoder.readBinaryElement(NDNProtocolDTags.Content);
	...
	public void encode(XMLEncoder encoder) throws ContentEncodingException {
		...
		encoder.writeElement(NDNProtocolDTags.Content, _content);

		encoder.writeEndElement();   		
	}

	@Override
	public long getElementLabel() { return NDNProtocolDTags.ContentObject; }
	...
	throws SignatureException, InvalidKeyException {
		try {
			return sign(name, signedInfo, content, offset, length,
							NDNDigestHelper.DEFAULT_DIGEST_ALGORITHM, signingKey);
		} catch (NoSuchAlgorithmException e) {
			if (Log.isLoggable(Level.WARNING))
				Log.warning("Cannot find default digest algorithm: " + NDNDigestHelper.DEFAULT_DIGEST_ALGORITHM);
			Log.warningStackTrace(e);
	...
	throws SignatureException, InvalidKeyException, NoSuchAlgorithmException {
		...
		try {
			...
			signature = 
				NDNSignatureHelper.sign(digestAlgorithm, 
						toBeSigned,
						signingKey);
	...
	public static boolean verify(ContentObject object,
		...
		if (null != contentProxy) {
			result = NDNSignatureHelper.verify(contentProxy, object.signature().signature(), object.signature().digestAlgorithm(), key);
		} else {
		...
		if ((!result) && Log.isLoggable(Log.FAC_VERIFY, Level.WARNING)) {
			Log.info("VERIFICATION FAILURE: " + object.name() + " timestamp: " + object.signedInfo().getTimestamp() + " content length: " + object.contentLength() + 
					...
					" to be signed sha256 digest: " + DataUtils.printHexBytes(NDNDigestHelper.digest(object.prepareContent())));
	...
	public static boolean verify(
		...
		boolean result = 
			NDNSignatureHelper.verify(preparedContent,
					signature.signature(),
					(signature.digestAlgorithm() == null) ? NDNDigestHelper.DEFAULT_DIGEST_ALGORITHM : signature.digestAlgorithm(),
							key);
		return result;
	...
	public static boolean verify(byte[] proxy, byte [] signature, SignedInfo signedInfo,
		...
		// Now, check the signature.
		boolean result = 
			NDNSignatureHelper.verify(proxy,
					signature,
					(digestAlgorithm == null) ? NDNDigestHelper.DEFAULT_DIGEST_ALGORITHM : digestAlgorithm,
							key);
		return result;
	}
	...
	public byte [] computeProxy() throws CertificateEncodingException, ContentEncodingException {
		...
		byte[] blockDigest = NDNDigestHelper.digest(
					prepareContent());
	...
	public static byte [] prepareContent(final ContentName name, final SignedInfo signedInfo, final byte [] content, int start, int length) throws ContentEncodingException {
		...
		encoder.writeElement(NDNProtocolDTags.Content, content, start, length);
	...
	protected byte[] calcDigest() {
		MessageDigest md;
		try {
			md = MessageDigest.getInstance(NDNDigestHelper.DEFAULT_DIGEST_ALGORITHM);
