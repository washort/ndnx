...
import org.ndnx.ndn.impl.security.keys.NetworkKeyManager;
import org.ndnx.ndn.impl.security.keys.RepositoryKeyManager;
...
/**
 * The standard NDNx mechanisms try to generate a keystore for each user that uses
 ...
 * 
 * As long as you are careful to create your NDNHandle objects pointing at these
...
public class CreateUserData {
	...
	/**
	 * Read/write constructor to write keystores as NDN data. 
	 ...
	 * @throws InvalidKeyException 
	 */
	public CreateUserData(ContentName userKeystoreDataPrefix, char [] password, NDNHandle handle) throws IOException, ConfigurationException, InvalidKeyException {
	...
	public void publishUserKeysToRepository() throws IOException{
		for (String friendlyName: _userKeyManagers.keySet()) {
			...
			NDNHandle userHandle = getHandleForUser(friendlyName);
			try {
	...
	public NDNHandle getHandleForUser(String friendlyName) throws IOException {
	 ...
	 */
	public static Tuple<Integer, NDNHandle> handleAs(String [] args, int offset) throws ConfigurationException, 
		...
		return new Tuple<Integer, NDNHandle>(t.first(), NDNHandle.open(t.second()));
	}
	...
	public static void usage() {
		System.out.println("usage: CreateUserData [[-f <file directory for keystores>] | [-r] <ndn uri for keystores>] [\"comma-separated user names\"] <user count> [<password>] [-p] (-r == use repo, -f == use files)");
	}
