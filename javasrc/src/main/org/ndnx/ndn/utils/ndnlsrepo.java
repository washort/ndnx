 ...
 *
 */
public class ndnlsrepo implements BasicNameEnumeratorListener {
	...
	/**
	 * Main function for the ndnlsrepo tool.  Initializes the tool and triggers name enumeration.
	 *
	...
	public static void main(String[] args) {
		...
		ndnlsrepo lister = new ndnlsrepo();
		lister.init(args);
	...
	/**
	 * Initialization function for ndnlsrepo.  This method parses the command line input
	 ...
	 * @return void
	 *
	 * @throws org.ndnx.ndn.protocol.MalformedContentNameStringException Converting the input to a
	 * ContentName can throw a MalformedContentNameException.
	 *
	 * @see org.ndnx.ndn.protocol.ContentName
	 */
	...
	/**
	 * Method to initialize a NDNHandle and the NDNNameEnumerator for the ndnlsrepo tool.
	 * This method also determines when the program should print out results and exit.
	 *
	 * @return void
	 *
	 * @throws org.ndnx.ndn.config.ConfigurationException A configuration exception is
	 * 	thrown if the NDNHandle is not properly configured
	 * @throws java.io.IOException  Am IOException is thrown if the NDNHandle is not properly initialized.
	 *
	 * @see org.ndnx.ndn.NDNHandle
	 * @see org.ndnx.ndn.profiles.nameenum.NDNNameEnumerator
	 */
	...
	private void enumerateNames() {
		try {
			...
			NDNNameEnumerator ndnNE = new NDNNameEnumerator(handle, this);
			ndnNE.registerPrefix(name);

			if (timeout > 0) {
				try {
					Thread.sleep(timeout);
				} catch (InterruptedException e) {
					System.err.println("error while waiting for responses from NDNNameEnumerator");
				}

				Log.fine("finished waiting for responses, cleaning up state");
				ndnNE.cancelPrefix(name);
				printNames();
			} else {
	...
	/**
	 * Function to print out the options for ndnlsrepo
	 *
	 * @returns void
	 */

	public void usage(String extraUsage) {
		System.out.println("usage: ndnlsrepo " + extraUsage + "<ndnprefix> [-timeout millis (default is 2000ms) | -c(ontinuous)]");
	}
	 ...
	 *
	 * @see org.ndnx.ndn.profiles.nameenum.BasicNameEnumeratorListener
	 */
