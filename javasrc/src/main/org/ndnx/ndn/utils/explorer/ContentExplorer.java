...
import org.ndnx.ndn.profiles.nameenum.BasicNameEnumeratorListener;
import org.ndnx.ndn.profiles.nameenum.NDNNameEnumerator;
 ...
 * ContentExplorer uses NDNNameEnumeration to populate the GUI and can open .txt
 ...
 * intended to be used as a first test of AccessControl functionality with NDN.
...
public class ContentExplorer extends JFrame implements BasicNameEnumeratorListener, ActionListener {
	...
	private NDNNameEnumerator _nameEnumerator = null;
	 ...
	 * The constructor also initializes the NDNHandle and name enumeration.
	 */
	public ContentExplorer() {
		super("NDN Content Explorer");
		if (userName != null) this.setTitle("NDN Content Explorer for " + userName);
				 ...
				 */
					...
					try {
						...
						while (temp==null) {
							...
							try {
								if (name.startsWith("ndn:/"))
									name = name.replaceFirst("ndn:/", "/");
								...
								if(name.equals(""))
									JOptionPane.showMessageDialog(chooser, "Please enter a NDNx name for the content that starts with \"/\".");
								else
									JOptionPane.showMessageDialog(chooser, (name + " is not a valid NDNx name.  Please be sure it starts with \"/\""));
							}
						}
	 ...
	 * @param ndnName
	 */
	public void sendFile(File file, ContentName ndnName) {

		htmlPane.setText("Writing " + file.getName() + " to NDN as: " + ndnName);

		ContentWriter writer = new ContentWriter(_handle, ndnName, file, htmlPane);
	 ...
	 *
	 * @param ndnContentName Prefix to retrieve in the tree.
	 * @return DefaultMutableTreeNode The node in the tree representing the supplied prefix.
	 */
	DefaultMutableTreeNode getTreeNode(ContentName ndnContentName) {
		Log.fine("handling returned names!!! prefix = "+ ndnContentName.toString());

		TreePath prefixPath = new TreePath(usableRoot);

		Log.fine("prefix path: " + prefixPath.toString());

		String[] names = new String[ndnContentName.count()];
		int ind = 0;
		for (byte[] n : ndnContentName) {
	...
	class MouseActions implements MouseListener {
		 ...
		 * The file is obtained through NDN.  If the file is no longer available, a message will appear in
		 * the preview pane.
		 *
	...
	/**
	 * Method to handle NDNNameEnumeration callbacks.  This implementation assumes the
	 * application handles duplicates. This method creates an instance of the Runnable
	 * AddChildren class to process the names returned through NDNNameEnumeration.
	 *
	...
	/**
	 * Method to register a prefix for name enumeration with NDNNameEnumerator
	 *
	...
	/**
	 * Method to return the NDNNameEnumerator for the ContentExplorer application
	 *
	 * @return NDNNameEnumerator
	 */
	public NDNNameEnumerator getNameEnumerator() {
		return _nameEnumerator;
	}

	/**
	 * Method to check for NDN Plugin for VLC.  Returns true if the ndn plugin is
	 * installed for VLC.  If it is not found, the "Play File" option is disabled
	 * for files.
	 *
	 * Currently not tested on non-linux platforms.
	 *
	 * @return  boolean True if the text ndn is found in the vlc --list output.
	 */
	...
	public boolean checkVLCsupport() {
		...
		String[] cmd = {"/bin/sh", "-c", "vlc --list | grep ndn"};
		try {
			...
			while ((line = brCleanUp.readLine ()) != null) {
				...
				if(line.toLowerCase().contains("ndn")) {
					check = true;
					Log.fine("ContentExplorer found NDN VLC plugin, enabling play option");
				}
			}
			brCleanUp.close();
			...
			Log.warning("ContentExplorer could not check for NDN VLC plugin, disabling play file option");
			Log.logException("Error checking for VLC NDN plugin", e);
		}
