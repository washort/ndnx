...
public class VersionNumberTest {
	...
	public void testVersionNumber()  throws Exception {
		...
		NDNTime t = NDNTime.now();
		Assert.assertTrue( t.getTime() - vn.getAsMillis() < 10);
	}

	@Test
	public void testVersionNumberNDNTime() {
		NDNTime t = NDNTime.now();
	...
	public void testVersionNumberLong() throws Exception {
		...
		for(int i = 0; i < 10000; i++) {
			...
			// tests on the value "v" will not always work, because NDNTime has
	...
	public void testVersionNumberByteArray() {
		...
		for(int i = 0; i < 100000; i++) {
			...
			NDNTime then = NDNTime.fromBinaryTimeAsLong(t);
	...
	public void testCompareTo() {
		...
		for(long vv : values) {
			NDNTime tt = NDNTime.fromBinaryTimeAsLong(vv);
	...
	public void testEqualsObject() {
		NDNTime now = NDNTime.now();
		long t = now.toBinaryTimeAsLong();
		NDNTime prev = NDNTime.fromBinaryTimeAsLong(t);
		for(int i = 0; i < 10000; i++) {
			t += _rnd.nextInt(100000) + 1;
			NDNTime next = NDNTime.fromBinaryTimeAsLong(t);
			NDNTime next2 = NDNTime.fromBinaryTimeAsLong(t);
	...
	@Test
	public void testBeforeNDNTime() {
	}

	@Test
	public void testAfterNDNTime() {
	}

//	@Test
//	public void testBinaryTime() throws Exception {
//		NDNTime start = new NDNTime(1294600000000L);
//
//		long current = start.toBinaryTimeAsLong();	
//
//		for(int i = 0; i < 100000000; i++) {
//			current++;
//			NDNTime t_current = NDNTime.fromBinaryTimeAsLong(current);
//			Assert.assertEquals(current, t_current.toBinaryTimeAsLong());
//
//			for(int j = 1; j <= 4; j++) {
//				long next = current + j;
//				NDNTime t_next = NDNTime.fromBinaryTimeAsLong(next);
//
...
//
//					NDNTime verify_current = VersioningProfile.getVersionComponentAsTimestamp(bytes_current);
//					NDNTime verify_next = VersioningProfile.getVersionComponentAsTimestamp(bytes_next);
...
//	}
	...
	// this was to look at the behavior of NDNTime for a specific version
//	@Test
...
//		
//		NDNTime t = VersioningProfile.getVersionComponentAsTimestamp(component.lastComponent());
//		
