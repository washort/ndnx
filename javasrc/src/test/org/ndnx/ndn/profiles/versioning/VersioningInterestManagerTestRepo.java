...
import org.ndnx.ndn.profiles.versioning.VersioningHelper.SinkHandle;
import org.ndnx.ndn.profiles.versioning.VersioningHelper.TestListener;
import org.ndnx.ndn.profiles.versioning.VersioningHelper.TestVIM;
...
public class VersioningInterestManagerTestRepo {
	...
	protected NDNHandle realhandle = null;
	...
	public void testStreamFromRight() throws Exception {
		...
		TreeSet<NDNTime> sent1 = sendStreamRight(sinkhandle, vim, basename, t, tosend);
		...
		TreeSet<NDNTime> sent2 = sendStreamRight(sinkhandle, vim, basename, t, 1);
	...
	public void testLongStreamFromLeft() throws Exception {
		...
		System.out.println("***** Sending stream 1 *****");
		TreeSet<NDNTime> sent1 = sendStreamLeft(sinkhandle, vim, basename, t, tosend);
	...
	private TreeSet<NDNTime> sendStreamGaussian(NDNHandle handle, TestVIM vim, ContentName name, double mean_time, double std_time, int count) throws Exception {
		...
		for(int i = 0; i < count; i++) {
			...
			do {
				...
				version = new NDNTime(Math.round(d));
	...
	private TreeSet<NDNTime> sendStreamUniform(NDNHandle handle, TestVIM vim, ContentName name, long start_time, long stop_time, int count) throws Exception {
		TreeSet<NDNTime> sent = new TreeSet<NDNTime>();
	...
	private TreeSet<NDNTime> sendStreamRight(NDNHandle handle, TestVIM vim, ContentName name, long t, int count) throws Exception {
		TreeSet<NDNTime> sent = new TreeSet<NDNTime>();

		for(int i = 0; i < count; i++) {
			// walk backwares from 10 msec to 100 sec
			t -= _rnd.nextInt(100000) + 10;
			NDNTime version = new NDNTime(t);
			sent.add(version);		
			send(handle, vim, name, version);
		}
		return sent;
	...
	private TreeSet<NDNTime> sendStreamLeft(NDNHandle handle, TestVIM vim, ContentName name, long t, int count) throws Exception {
		TreeSet<NDNTime> sent = new TreeSet<NDNTime>();
	...
	private void send(NDNHandle handle, TestVIM vim, ContentName name, NDNTime version) throws IOException, InterruptedException {
