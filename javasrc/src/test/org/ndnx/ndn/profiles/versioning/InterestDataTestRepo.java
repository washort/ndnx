...
public class InterestDataTestRepo {
	...
	public void testVersionNumberCompare() throws Exception {
		...
		VersionNumber a = new VersionNumber(new NDNTime(111111000000L));
		VersionNumber aa = new VersionNumber(new NDNTime(111111000000L));
		VersionNumber b = new VersionNumber(new NDNTime(111222000000L));
		VersionNumber c = new VersionNumber(new NDNTime(111333000000L));
	...
	public void testInterestDataInterest() throws Exception {
		...
		// Save content
		NDNStringObject so = new NDNStringObject(basename, "hello, world!", SaveType.LOCALREPOSITORY, handle);
		so.save();
		NDNTime version = so.getVersion();
		so.close();
		...
		NDNTime received_version = VersioningProfile.getLastVersionAsTimestamp(co.name());
		Assert.assertTrue(version.equals(received_version));

		NDNStringObject received_so = new NDNStringObject(co, handle);
	...
	public void testInterestDataInterestStream() throws Exception {
		...
		ArrayList<NDNStringObject> sent = VersioningHelper.sendEventStream(handle, basename, tosend);
	...
	public void testInterestDataInterestStreamWithStartTime() throws Exception {
		...
		// Send a stream of string objects
		ArrayList<NDNStringObject> sent1 = VersioningHelper.sendEventStream(handle, basename, tosend);
		VersionNumber cutoff_version = new VersionNumber(sent1.get(sent1.size()-1).getVersion());
		
		// now send another stream
		ArrayList<NDNStringObject> sent2 = VersioningHelper.sendEventStream(handle, basename, tosend);
