...
import org.ndnx.ndn.profiles.security.access.group.ACL;
...
public class MLACReadWriteTestRepo {
	...
	public static void setUpBeforeClass() throws Exception {
		...
		for (int d=0; d<domainCount; d++) {
			domainPrefix[d] = ContentName.fromNative("/ndnx.org/domain" + d);
		...
		NDNTestHelper testHelper = new NDNTestHelper(MLACReadWriteTestRepo.class);
		...
		GroupAccessControlManager.create(baseDirectory, profileName, rootACL, parameterizedNames, null, SaveType.REPOSITORY, NDNHandle.getHandle());
	...
	public void writeContentInSubdirectory() {
		...
		try {
			...
			NDNOutputStream ostream = new RepositoryFileOutputStream(nodeName, _AliceHandle);
	...
	public void readFileAs(int domain, String userName) throws Exception {
		...
		try {
			...
			NDNInputStream input = new NDNFileInputStream(nodeName, handle);
