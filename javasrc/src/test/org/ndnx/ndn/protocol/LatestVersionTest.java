...
public class LatestVersionTest {
	...
	static NDNHandle getHandle;
	static NDNHandle responderHandle;
	...
	public void setUp() throws Exception {
		...
		getHandle = NDNHandle.open();
		baseName = ContentName.fromURI("/ndnx.org/test/latestVersionTest/"+(new NDNTime()).toShortString());
	...
	/**
	 * Test to check if the getLatestVersion method in VersioningProfile gets the latest version with a ndnd involved.
	 * @throws Error
	...
	public void getLatestVersion() throws InterruptedException, Error {
		...
		try {
			//should not force content objects into ndnd...
		...
		NDNTime versionToAdd = new NDNTime();
		...
		// putting the test objects to ndnd. But once we have a verifier, there's a potential race between the verifier and responder i.e.
	 ...
	 */
	class Responder implements NDNInterestHandler {
		AssertionNDNHandle handle;

		public Responder() throws IOException {
			try {
				handle = AssertionNDNHandle.open();
