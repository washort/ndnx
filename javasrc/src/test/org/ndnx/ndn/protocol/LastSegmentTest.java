...
/**
 * This set of tests checks the implementation of getLastSegment with ndnd involved.
 */
public class LastSegmentTest {
	...
	public void lastSegmentTestCheckTimeout(){
		...
		long stoptime;
		try {
			timeoutName = ContentName.fromURI("/ndnx.org/test/segmentationProfile/baseName/checkTimeout");
	...
	public void testIsLastSegment(){
		...
		try {
			name = ContentName.fromURI("/ndnx.org/test/segmentationProfile/testIsLastSegment/");
	...
	public void testIsNotLastSegment(){
		...
		ContentName name = null;
		try {
			name = ContentName.fromURI("/ndnx.org/test/segmentationProfile/testIsNotLastSegment/");
			name = VersioningProfile.addVersion(name);
	...
	public void testGetLastSegmentWithMultipleSegmentsUnversioned() {
		...
		try {
			name = ContentName.fromURI("/ndnx.org/test/segmentationProfile/"+(new NDNTime()).toShortString()+"/testGetLastMultiUnversioned/");
			one = name;
			last = one;
	...
	public void testGetLastSegmentWithMultipleSegmentsVersioned() {
		...
		try {
			name = ContentName.fromURI("/ndnx.org/test/segmentationProfile/"+(new NDNTime()).toShortString()+"/testGetLastMultiVersioned/");
			name = VersioningProfile.addVersion(name);
			one = name;
			last = one;
			one = SegmentationProfile.segmentName(one, SegmentationProfile.baseSegment());
	...
	public void testGetLastSegmentWithSingleSegmentVersioned() {
		...
		try {
			name = ContentName.fromURI("/ndnx.org/test/segmentationProfile/"+(new NDNTime()).toShortString()+"/testGetLastSingleVersioned/");
			name = VersioningProfile.addVersion(name);
			one = name;
			one = SegmentationProfile.segmentName(one, SegmentationProfile.baseSegment());
		} catch (MalformedContentNameStringException e) {
