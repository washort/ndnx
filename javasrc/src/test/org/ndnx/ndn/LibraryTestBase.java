 ...
 * between different threads or objects, via ndnd.
 * New tests should probably not use this without some additional cleanup.
 *
 */
public class LibraryTestBase extends NDNTestBase {
	 ...
	 * @throws InterruptedException 
	 */
	public void getResults(ContentName baseName, int count, NDNHandle handle) throws IOException, InvalidKeyException, SignatureException, InterruptedException {
...
//				done = true; 
	...
	public class GetThread implements Runnable {
		protected NDNHandle handle = null;
		int count = 0;
		int id = 0;
		public GetThread(int n, int id) throws ConfigurationException, IOException {
			...
			if (DO_TAP) {
				try {
					((NDNHandle)handle).getNetworkManager().setTap(SystemConfiguration.DEBUG_DATA_DIRECTORY + "/LibraryTestDebug_" + Integer.toString(id) + "_get");
	...
	public class PutThread implements Runnable {
		...
		public PutThread(int n, int id) throws ConfigurationException, IOException {
			...
			if (DO_TAP) {
				try {
					((NDNHandle)handle).getNetworkManager().setTap(SystemConfiguration.DEBUG_DATA_DIRECTORY + "/LibraryTestDebug_" + Integer.toString(id) + "_put");
				} catch (IOException ie) {
				}
			}
		}
	...
	public class GetServer implements Runnable, NDNContentHandler {
		protected NDNHandle handle = null;
		int count = 0;
		int next = 0;
	...
	public class PutServer implements Runnable, NDNInterestHandler {
		protected NDNHandle handle = null;
		int count = 0;
		int next = 0;
		Semaphore sema = new Semaphore(0);
		...
		HashSet<Integer> accumulatedResults = new HashSet<Integer>();
		int id;
		NDNFlowControl cf = null;
		NDNWriter writer = null;
		...
		public void run() {
			try {
				...
				writer = new NDNWriter(name, handle);
