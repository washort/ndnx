...
import org.ndnx.ndn.impl.security.crypto.util.SignatureHelper;
...
public class BenchmarkTest {
	...
	public static NDNTestHelper testHelper = new NDNTestHelper(BenchmarkTest.class);
	public static NDNHandle handle;
	...
	public static final String [] digestAlgorithms = new String[]{"MD5", "SHA1", NDNDigestHelper.DEFAULT_DIGEST_ALGORITHM};
	...
	@Test
	public void testNdndRetrieve() throws Exception {
		// Floss some content into ndnd
		ContentName dataPrefix = testHelper.getTestNamespace("TestNdndRetrieve");

		Flosser floss = new Flosser(dataPrefix);
		NDNStringObject so = new NDNStringObject(dataPrefix, "This is the value", SaveType.RAW, handle);
		so.save();
		ContentName name = so.getVersionedName();
		so.close();
		floss.stop();
		
		// Now that content is in local ndnd, we can benchmark retrieval of one content item
		Operation<Interest, Object> getcontent = new Operation<Interest, Object>() {
			Object execute(Interest interest, Object ignored) throws Exception {
				...
				ContentObject result = handle.get(interest, NDNNetworkManager.PERIOD);
		...
		System.out.println("==== Single data retrieval from ndnd: " + name);
