...
import org.ndnx.ndn.protocol.BloomFilter;
...
public class ReadTest extends LibraryTestBase implements NDNContentHandler {
	...
	public void getNextTest() throws Throwable {
		...
		NDNWriter writer = new NDNWriter("/getNext", putHandle);
		NDNReader reader = new NDNReader(getHandle);
		for (int i = 0; i < count; i++) {
			writer.put("/getNext/" + Integer.toString(i), Integer.toString(count - i));
			Thread.sleep(rand.nextInt(50));
			
			// Pull it into ndnd so we have everything there to check nexts from
	...
	public void getLatestTest() throws Throwable {
		...
		int highest = 0;
		NDNWriter writer = new NDNWriter("/getLatest", putHandle);
		NDNReader reader = new NDNReader(getHandle);
		for (int i = 0; i < count; i++) {
			...
			// Make sure ndnd has what we're looking for
			Thread.sleep(500);
	...
	public void excludeTest() throws Throwable {
		...
		excludeSetup();
		NDNWriter writer = new NDNWriter("/excludeTest", putHandle);
		for (String value : bloomTestValues) {
	...
	private void excludeTest(String prefix, int nFilters) throws Throwable {
		...
		NDNWriter writer = new NDNWriter(prefix, putHandle);
