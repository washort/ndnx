...
import org.ndnx.ndn.impl.security.crypto.MerkleTree;
...
public class MerkleTreeTest {
	...
	public static byte [][] makeContent(int numNodes, int nodeLength, boolean digest) {
		...
		int blocklen = (digest ? NDNDigestHelper.DEFAULT_DIGEST_LENGTH  : nodeLength);
		...
		for (int i=0; i < numNodes; ++i) {
			...
			if (digest) {
				...
				bufs[i] = NDNDigestHelper.digest(tmpbuf);
			} else {
