...
package org.ndnx.ndn.security.crypto;
...
/**
 * Test NDN-specific Merkle tree functionality.
 */
public class NDNMerkleTreeTest {
	...
	public void testMerkleTree() throws Exception {
		...
		try {
			...
			Assert.fail("NDNMerkleTree should throw an exception for tree sizes < 2.");
...
//		int [] nodecounts = new int[]{1000,1001,1025,1098,1536,1575,2053,5147,8900,9998,9999,10000};
	...
	public static void testTree(int nodeCount, int blockWidth, boolean randomWidths) throws Exception {
		...
		try {
			...
			NDNMerkleTree tree = new NDNMerkleTree(cos,
													pair.getPrivate());
			tree.setSignatures();
			...
			for (int i=0; i < tree.numLeaves()-1; ++i) {
				...
				if (!result) {
					...
					byte [] digest = NDNDigestHelper.digest(block.encode());
					byte [] tbsdigest = 
						NDNDigestHelper.digest(block.prepareContent());
					System.out.println("Raw content digest: " + DataUtils.printBytes(NDNDigestHelper.digest(block.content())) +
							" object content digest:  " + DataUtils.printBytes(NDNDigestHelper.digest(block.content())));
