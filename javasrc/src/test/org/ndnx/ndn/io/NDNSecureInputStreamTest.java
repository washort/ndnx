...
import org.ndnx.ndn.impl.security.crypto.StaticContentKeys;
 ...
 */
public class NDNSecureInputStreamTest {
	...
	static protected abstract class StreamFactory {
		...
		public abstract NDNInputStream makeInputStream() throws IOException;
		...
		public void streamEncryptDecrypt() throws IOException {
			...
			NDNInputStream vfirst = makeInputStream();
		...
		public void seekZero() throws IOException, NoSuchAlgorithmException {
			NDNInputStream i = makeInputStream();
			i.seek(0);
		}
		...
		private void readAndCheck(NDNInputStream i, int start, int length)
		...
		public void skipping() throws IOException, NoSuchAlgorithmException {
			...
			NDNInputStream inStream = makeInputStream();
	 ...
	 */
	static NDNTestHelper testHelper = new NDNTestHelper(NDNSecureInputStreamTest.class);

	static NDNHandle outputLibrary;
	static NDNHandle inputLibrary;
	...
	public static void setUpBeforeClass() throws Exception {
		outputLibrary = NDNHandle.open();
		inputLibrary = NDNHandle.open();

		flosser = new Flosser();

		basic = new StreamFactory("basic.txt", 25*1024+301){
			public NDNInputStream makeInputStream() throws IOException {
				return new NDNInputStream(name, null, null, keys, inputLibrary);
			}
			public OutputStream makeOutputStream() throws IOException {
				return new NDNOutputStream(name, null, null, null, keys, outputLibrary);
			}
		};

		versioned = new StreamFactory("versioned.txt", 25*1024+302){
			public NDNInputStream makeInputStream() throws IOException {
				return new NDNVersionedInputStream(name, 0L, null, keys, inputLibrary);
			}
			public OutputStream makeOutputStream() throws IOException {
				return new NDNVersionedOutputStream(name, null, null, keys, outputLibrary);
			}
		};

		file = new StreamFactory("file.txt",  25*1024+303){
			public NDNInputStream makeInputStream() throws IOException {
				return new NDNFileInputStream(name, null, null, keys, inputLibrary);
			}
			public OutputStream makeOutputStream() throws IOException {
				return new NDNFileOutputStream(name, keys, outputLibrary);
			}
		};
