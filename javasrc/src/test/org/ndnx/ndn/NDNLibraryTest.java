...
import org.ndnx.ndn.BasicInterestListener;
import org.ndnx.ndn.NDNBase;
 ...
 */
public class NDNLibraryTest extends LibraryTestBase {
	...
	public void testEnumerate() {
		...
		try {
			...
			NDNWriter writer = new NDNWriter("/CPOF", putHandle);
...
//		SignedInfo.ContentType type = SignedInfo.ContentType.LEAF;
		...
		try {
			NDNWriter segmenter = new NDNWriter(name, putHandle);
	...
	public void testRevision() {
		...
		try {
			...
			NDNWriter segmenter = new NDNWriter(keyName, putHandle);
	...
	public void testGetLatestVersion() throws Exception {
		...
		NDNFlowControl f = new NDNFlowControl(base, putHandle);
		...
		class t {
			...
			/**
			 * Make sure the data is written to ndnd by reading it
	...
	public void testRecall() {
		...
		NDNTime time = NDNTime.now();
		try {
			ContentName keyName = ContentName.fromNative(key);
			NDNWriter writer = new NDNWriter(keyName, putHandle);
	...
	public void versionTest(ContentName docName,
		...
		NDNWriter writer = new NDNWriter(docName, putHandle);
	...
	class TestListener extends BasicInterestListener {
		...
		public TestListener(NDNBase queryProvider,
			...
			try {
				if (null != co) {
						...
						if (VersioningProfile.hasTerminalVersion(co.name())) {
							// We're writing this content using NDNWriter.put. That interface
	...
	public void testInterest() {
		...
		try {
			NDNWriter writer = new NDNWriter(key, putHandle);
