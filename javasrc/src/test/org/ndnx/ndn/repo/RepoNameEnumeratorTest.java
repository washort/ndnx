 ...
 */
public class RepoNameEnumeratorTest implements BasicNameEnumeratorListener, NDNContentHandler {
	
	static final long WAIT_TIME = 500;

	NDNHandle getLibrary;
	NDNNameEnumerator getne;
	...
	NDNHandle putLibrary;
	AssertionNDNHandle explicitExcludeHandle;
	...
	public void setHandles(){
		try {
			getLibrary = NDNHandle.open();
			getne = new NDNNameEnumerator(getLibrary, this);
			
			putLibrary = NDNHandle.open();
	...
	public void explicitExcludeFastResponseTest(){
		...
		try {
			explicitExcludeHandle = AssertionNDNHandle.open();
	...
	public Interest handleContent(ContentObject data, Interest interest) {
		...
		//for now, this is copied from NDNNameEnumerator.getIDFromName  should be refactored so it isn't manually copied.
		try {
