...
public class MultiResponderNameEnumerationTest implements BasicNameEnumeratorListener {
	...
	NDNNameEnumerator putne;
	
	NDNHandle _putHandle;
	NDNHandle _getHandle;
	
	NDNHandle _fileHandle;
	
	NDNTestHelper helper;
	...
	public void setUp() {
		try {
			...
			_putHandle = NDNHandle.open();
			_getHandle = NDNHandle.open();
			_fileHandle = NDNHandle.open();
			
			getne = new NDNNameEnumerator(_getHandle, this);
			
			putne = new NDNNameEnumerator(_putHandle, this);
			
			helper = new NDNTestHelper(this.getClass().getName());
