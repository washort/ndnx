...
public class RFSTest extends RepoTestBase {
	...
	public void testPolicy() throws Exception {
		...
		try {	// Test no version
			repo.initialize(_fileTestDir, new File(_topdir + "/org/ndnx/ndn/repo/badPolicyTest1.xml"), null, null, null, null);
			Assert.fail("Bad policy file succeeded");
		} catch (RepositoryException re) {}
		try {	// Test bad version
			repo.initialize(_fileTestDir, new File(_topdir + "/org/ndnx/ndn/repo/badPolicyTest2.xml"), null, null, null, null);
			Assert.fail("Bad policy file succeeded");
		} catch (RepositoryException re) {}
		// Make repository using repo's keystore, not user's
		repo.initialize(_fileTestDir,  
					new File(_topdir + "/org/ndnx/ndn/repo/policyTest.xml"), _repoName, _globalPrefix, null, null);
		repo.shutDown();
