...
As the access control support in NDN is still relatively early
...
We represent wrapped keys as ndnb-encoded XML objects. 
...
We define $\mbox{Data}_{\Obj}$ as the concatenation of the ndnb
...
In NDNx, groups are stored as special nodes in the name
...
\ndnxpath{<protected namespace root>/Users/}

Where \ndnxpath{<protected namespace root>} is the root of the
...
\ndnxpath{<protected namespace root>/Users/<username>}\\
...
\ndnxpath{GROUP\_PREFIX := <protected namespace root>/Groups/}\\
...
\ndnxpath{GROUP\_PREFIX := <protected namespace root>/Groups/<Group Friendly Name (UTF-8)>}\\
...
\ndnxpath{<GROUP\_PREFIX>/<Group Friendly Name (UTF-8)>/Members/<version marker>}\\
...
\ndnxpath{<GROUP\_PREFIX>/<Group Friendly Name (UTF-8)>/KEY/<version marker>/<segmentation>}\\
...
\ndnxpath{<GROUP\_PREFIX>/<Group Friendly Name (UTF-8)>/KEY/<version marker>/PrivateKey}\\
...
\ndnxpath{<GROUP\_PREFIX>/<Group Friendly Name (UTF-8)>/KEY/<version marker>/keyid:<KeyID>}\\
...
\ndnxpath{<GROUP\_PREFIX>/<Group Friendly Name (UTF-8)>/KEY/<version marker>/p:<PrincipalName>:<PrincipalVersion>}\\
...
\ndnxpath{<GROUP\_PREFIX>/<Group Friendly Name (UTF-8)>/KEY/<version marker>/PreviousKey} (optional) \\
...
\ndnxpath{<GROUP\_PREFIX>/<Group Friendly Name (UTF-8)>/KEY/<version marker>/SupersedingKey} (optional) \\
...
\ndnxpath{<N>/\_access\_/ACL/<version marker>}\\
...
\ndnxpath{/\_access\_/NK/} for the latest version of the node
...
\ndnxpath{<N>/\_access\_/NK/<version marker>/p:<PrincipalName>:<PrincipalVersion>}\\
contains a link from a friendly identifier of the target principal
(group friendly name or user friendly name as listed in the User or
Group namespace), and the version identifier (as 12-bit timestamp) of
their current key (at the time of wrapping) to the wrapped key block
...
\ndnxpath{<N>/\_access\_/NK/<version marker>/keyid:<key\_id>}\\
