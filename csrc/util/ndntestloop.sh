# Source file: util/ndntestloop.sh
# 
...
Usage () {
cat << EOM >&2
usage: ndntestloop [ start | stop | restart | status ]
With no argument test run in foreground
(Must be run from the top level of a ndnx source tree.)
EOM
exit 1
}

Defaults () {
NDN_TEST_GITCOMMAND=`command -v git || echo :`
test -d .git || NDN_TEST_GITCOMMAND=:

cat << EOF
# EDIT config, NOT config.defaults
# Control the overall log level of the java tests.
NDN_LOG_LEVEL_ALL=WARNING
...
NDN_TEST_BRANCH=HEAD
NDN_TEST_GITCOMMAND=$NDN_TEST_GITCOMMAND
...
CheckLogLevel () {
	case ${NDN_LOG_LEVEL_ALL} in
		...
		FINEST) ;;
		ALL) ;;
		*) Fail NDN_LOG_LEVEL_ALL=$NDN_LOG_LEVEL_ALL is invalid.
	esac
}

CheckDirectory () {
	test -d javasrc || Fail ndntestloop is intended to be run at the top level of ndnx
	test -d testdir/. || mkdir testdir
	mkdir -p testdir/hooks
}

BackgroundPID () {
	if [ -f testdir/ndntestloop.pid ]; then
		PID=`cat testdir/ndntestloop.pid`
...
SetPIDFile () {
	if [ -f testdir/ndntestloop.pid ]; then
		PID=`cat testdir/ndntestloop.pid`
		test "$PID" = $$ && return 0
		kill -0 "$PID" 2>/dev/null && Fail ndntestloop already running as pid $PID
		rm testdir/ndntestloop.pid || Fail could not remove old pid file
	fi
	echo $$ > testdir/ndntestloop.pid || Fail could not write pid file
}

RemovePIDFile () {
	test "`cat testdir/ndntestloop.pid`" = $$ && rm -f testdir/ndntestloop.pid
}
...
PrintDetails () {
	uname -a
	env | grep ^NDN
	$NDN_TEST_GITCOMMAND rev-list --max-count=1 HEAD
	$NDN_TEST_GITCOMMAND status
	$NDN_TEST_GITCOMMAND diff | cat
...
UpdateSources () {
	...
	cp csrc/util/ndntestloop.sh testdir/.~ctloop~
	if [ -x testdir/hooks/update ]; then
		testdir/hooks/update $1 || Fail testdir/hooks/update
		return 0	
	fi
	$NDN_TEST_GITCOMMAND status | grep modified:        && \
	  Echo Modifications present - skipping update      && \
	  sleep 3 && return
	$NDN_TEST_GITCOMMAND checkout $NDN_TEST_BRANCH      && \
	  $NDN_TEST_GITCOMMAND pull origin $NDN_TEST_BRANCH
}
...
ScriptChanged () {
	...
	diff testdir/.~ctloop~ csrc/util/ndntestloop.sh && return 1
...
LimitCPU () {
	test -z "$NDN_TEST_CPU_LIMIT" && return
	ulimit -t $NDN_TEST_CPU_LIMIT
}

RunCTest () {
	test "${NDN_CTESTS:=}" = "NO" && return 0
	Echo Running csrc tests...
	LOG=javasrc/testout/TEST-csrc-testlog.txt
	mkdir -p javasrc/testout
	(LimitCPU; cd csrc && $MAKE test TESTS="$NDN_CTESTS" 2>&1 ) > $LOG
	STATUS=$?
	NoteTimes postc
...
RunJavaTest () {
	test "${NDN_JAVATESTS:-}" = "NO" && return 0
	Echo Running javasrc tests...
	LOG=javasrc/testout/TEST-javasrc-testlog.txt
	(LimitCPU; cd javasrc && \
	  ant -DCHATTY=${NDN_LOG_LEVEL_ALL}             \
	      -DTEST_PORT=${NDN_LOCAL_PORT_BASE:-63000} \
	      ${NDN_JAVATESTS:-test}; ) > $LOG
	STATUS=$?
	NoteTimes postj
	test $STATUS -eq 0 && return 0
...
ExecSelf () {
	exec sh csrc/util/ndntestloop.sh || Fail exec
}

StartBackground () {
	BackgroundPID 'ndntestloop already running as pid ' && return 1
	sh -c "sh csrc/util/ndntestloop.sh &" < /dev/null >> testdir/log 2>&1
}
...
Status () {
	...
	BackgroundPID 'ndntestloop running as pid '
	STATUS=$?
	test $STATUS = 0 || echo ndntestloop not running
	return $STATUS
}
