/**
 * @file ndndumppcap.c
 * Reads in a one or more ndnbs from stdin (e.g. as dumped
 * by ndndump), and writes them to a pcap file. Uses the splitting
 * code from ndn_splitndnb.
 *
...
#define DEFAULT_DEST_PORT NDN_DEFAULT_UNICAST_PORT_NUMBER
...
usage(const char *progname)
{
    fprintf(stderr,
            "%s <infile> [<infile> ...]\n"
            "   Reads ndnb blocks from one or more files, and writes them in pcap format\n"
            "   to stdout.\n"
            "   ndnb blocks can be generated by any of the other utility programs.\n",
            progname);
    exit(1);
}
...
dump_udp_packet(pcap_dumper_t *dump_file, 
                ...
                const unsigned char *data, size_t data_len,  /* data; could be whole ndnb, could
                                                           just be contents */
...
main(int argc, char **argv)
{
    ...
    for (i = 1; argv[i] != 0; i++) {
        ...
        /* DKS -- eventually take IP addresses and ports from command line,
           as well as whether to dump only the ndn content. */
