...
usage(char *prog)
{
    fprintf(stderr,
            ...
            "   topo-uri, prefix-uri, and filter-uri must be NDNx URIs.\n"
...
int
sync_cb(struct ndns_name_closure *nc,
        ...
        struct ndn_charbuf *name)
{
    ...
    if (name != NULL)
        ...
        ndn_charbuf_append_string(uri, "(null)");
    printf("%s %s %s\n", ndn_charbuf_as_string(uri), hexL, hexR);
    fflush(stdout);
    free(hexL);
    free(hexR);
...
main(int argc, char **argv)
{
    ...
    struct ndns_name_closure nc = {0};
    struct ndns_name_closure *closure = &nc;
    struct ndn_charbuf *prefix = ndn_charbuf_create();
    struct ndn_charbuf *roothash = NULL;
    ...
    while ((opt = getopt(argc, argv, "hf:p:r:t:w:")) != -1) {
        switch (opt) {
            case 'f':
                ...
                if (0 > ndn_name_from_uri(clause, optarg)) usage(argv[0]);
                ndns_slice_add_clause(slice, clause);
                break;
            case 'p':
                ndn_charbuf_reset(prefix);
                if (0 > ndn_name_from_uri(prefix, optarg)) usage(argv[0]);
                break;
            case 'r':
                n = strlen(optarg);
                if (n == 0) {
                    roothash = ndn_charbuf_create();
                    break;
                }
                if ((n % 2) != 0)
                    usage(argv[0]);
                roothash = ndn_charbuf_create_n(n / 2);
                for (i = 0; i < (n / 2); i++) {
                    j = (hex_value(optarg[2*i]) << 4) | hex_value(optarg[1+2*i]);
                    ndn_charbuf_append_value(roothash, j, 1);
                }
                break;
            case 't':
                ndn_charbuf_reset(topo);
                if (0 > ndn_name_from_uri(topo, optarg)) usage(argv[0]);
                break;
            case 'w':
                timeout = atoi(optarg);
    ...
    ndns_slice_set_topo_prefix(slice, topo, prefix);
    h = ndn_create();
    res = ndn_connect(h, NULL);
    closure->callback = &sync_cb;
    ndns = ndns_open(h, slice, closure, roothash, NULL);
    ndn_run(h, timeout);
    ndns_close(&ndns, NULL, NULL);
