/**
 * @file ndnhexdumpdata.c
 * Parse a ndnb-encoded ContentObject in a file and dump the content as hex data to stdout.
 *
 ...
 retry:
    ...
    if (d->state < 0) {
        ...
        if (s < n) {
            content_value = NULL;
            content_length = 0;
            if (ndn_parse_ContentObject(data, s, &content, comps) != 0) {
                fprintf(stderr, "unable to parse content object\n");
                res = 1;
            }
            else if (ndn_content_get_value(data, s, &content, &content_value, &content_length) != 0) {
