/**
 * @file ndn_xmltondnb.c
 * Utility to convert XML into ndn binary encoded data (ndnb).
 *
...
usage(const char *progname)
{
    fprintf(stderr,
            ...
            " otherwise output files get .ndnb extension\n",
            progname);
    exit(1);
}

struct ndn_encoder_stack_item {
    size_t start;
    size_t end;
    struct ndn_encoder_stack_item *link;
};

struct ndn_encoder {
    struct ndn_charbuf *openudata;
    int is_base64binary;
    int is_hexBinary;
    int is_text;
    int toss_white;
...
static int
dict_lookup(const char *key, const struct ndn_dict_entry *dict, int n)
{
    int i;
    for (i = 0; i < n; i++)
...
struct ndn_encoder *
ndn_encoder_create(FILE *outfile, const struct ndn_dict *dtags)
{
    struct ndn_encoder *c;
    c = calloc(1, sizeof(*c));
    if (c) {
        c->openudata = ndn_charbuf_create();
        if (c->openudata != NULL)
            ndn_charbuf_reserve(c->openudata, 128);
        c->outfile = outfile;
...
void
ndn_encoder_destroy(struct ndn_encoder **cbp)
{
    struct ndn_encoder *c = *cbp;
    if (c != NULL) {
        ndn_charbuf_destroy(&c->openudata);
        free(c);
        *cbp = NULL;
    }
}
...
static void
emit_bytes(struct ndn_encoder *u, const void *p, size_t length)
{
...
static void
emit_tt(struct ndn_encoder *u, size_t numval, enum ndn_tt tt)
{
    unsigned char buf[1+8*((sizeof(numval)+6)/7)];
    unsigned char *p = buf + (sizeof(buf)-1);
    int n = 1;
    p[0] = (NDN_TT_HBIT & ~NDN_CLOSE) |
           ((numval & NDN_MAX_TINY) << NDN_TT_BITS) |
           (NDN_TT_MASK & tt);
    numval >>= (7-NDN_TT_BITS);
    while (numval != 0) {
        (--p)[0] = (((unsigned char)numval) & ~NDN_TT_HBIT) | NDN_CLOSE;
        n++;
        numval >>= 7;
    }
    emit_bytes(u, p, n);
...
static int
all_whitespace(struct ndn_charbuf *b)
{
    size_t i;
    size_t n = b->length;
    for (i = 0; i < n; i++) {
...
static void
finish_openudata(struct ndn_encoder *u)
{
    if (u->is_base64binary) {
        ...
        u->is_base64binary = 0;
        obuf = ndn_charbuf_reserve(u->openudata, maxbinlen);
        if (obuf != NULL) {
            d.output = obuf;
        ...
        u->is_text = 0;
        emit_tt(u, u->openudata->length, NDN_BLOB);
    ...
    if (u->openudata->length != 0) {
        if (!(u->toss_white && all_whitespace(u->openudata))) {
            emit_tt(u, u->openudata->length, NDN_UDATA);
            emit_bytes(u, u->openudata->buf, u->openudata->length);
        }
        u->openudata->length = 0;
    }
...
static void
emit_name(struct ndn_encoder *u, enum ndn_tt tt, const void *name)
{
...
static void
emit_xchars(struct ndn_encoder *u, const XML_Char *xchars)
{
    size_t length = strlen(xchars);
    finish_openudata(u);
    emit_tt(u, length, NDN_UDATA);
    emit_bytes(u, xchars, length);
}

static void
emit_closer(struct ndn_encoder *u)
{
    static const unsigned char closer[] = { NDN_CLOSE };
    finish_openudata(u);
...
do_start_element(void *ud, const XML_Char *name,
                 const XML_Char **atts)
{
    struct ndn_encoder *u = ud;
    const XML_Char **att;
    int is_base64binary = 0;
    int is_hexBinary = 0;
    int is_text = 0;
    emit_name(u, NDN_TAG, name);
    for (att = atts; att[0] != NULL; att += 2) {
        if (0 == strcmp(att[0], "ndnbencoding")) {
            ...
            fprintf(stderr, "warning - unknown ndnbencoding found (%s)\n", att[1]);
        }
        emit_name(u, NDN_ATTR, att[0]);
        emit_xchars(u, att[1]);
    }
...
{
     struct ndn_encoder *u = ud;
     ndn_charbuf_append(u->openudata, s, len);
}
...
do_processing_instructions(void *ud, const XML_Char *target, const XML_Char *data)
{
     struct ndn_encoder *u = ud;
     finish_openudata(u);
     emit_tt(u, NDN_PROCESSING_INSTRUCTIONS, NDN_EXT);
...
#define TOSS_WHITE 1
static int
process_fd(int fd, FILE *outfile, int flags, const struct ndn_dict *dtags)
{
    char buf[17];
    ssize_t len;
    int res = 0;
    struct ndn_encoder *u;
    XML_Parser p;
    u = ndn_encoder_create(outfile, dtags);
    if (u == NULL) return(1);
    if (flags & TOSS_WHITE) {
        u->toss_white = 1;
    }
    ...
    XML_ParserFree(p);
    ndn_encoder_destroy(&u);
    
    return(res);
}

static int
process_file(char *path, int flags, const struct ndn_dict *dtags)
{
    int fd = 0;
    int res = 0;
    FILE *outfile = stdout;
...
main(int argc, char **argv)
{
    ...
    for (i = 1; argv[i] != 0; i++) {
        ...
        if (0 == strcmp(argv[i], "-d")) {
            if (argv[i+1] != 0) {
                if (0 > ndn_extend_dict(argv[i+1], dtags, &dtags)) {
