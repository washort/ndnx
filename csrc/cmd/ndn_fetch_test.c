/**
 * @file ndn_fetch_test.c
 * @brief Provides a test platform for ndn_fetch.
 *
...
NewElem(struct MyParms *p) {
	...
	MyCharbuf cbName = ndn_charbuf_create();
	int res = ndn_name_from_uri(cbName, name);
	if (res < 0) {
		fprintf(stderr, "** open of %s failed!\n", name);
	} else {
		e->fs = ndn_fetch_open(p->f, cbName,
							   name,
							   NULL,
							   p->maxSegs,
							   p->resolveVersion,
...
runTest(struct MyParms *p) {
	...
	if (e->fs == NULL) {
		...
		for (;;) {
			...
			sds.fdLen = p->ndnFD+1;
			FD_SET(p->ndnFD, &sds.readFDS);
			FD_SET(p->ndnFD, &sds.writeFDS);
			FD_SET(p->ndnFD, &sds.errorFDS);
			...
			if (res != 0) ndn_fetch_poll(p->f);
			intmax_t nb = ndn_fetch_read(e->fs, e->buf, e->bufMax);
			if (nb == NDN_FETCH_READ_END) {
				// end of this test
				break;
			} else if (nb > 0) {
				...
				MilliSleep(5);
			} else if (nb == NDN_FETCH_READ_TIMEOUT) {
				// timeouts are treated as transient (maybe not true)
				ndn_reset_timeout(e->fs);
				MilliSleep(5);
			} else {
				// random failure
				msg = "read failed";
...
static char *help = "usage: ndn_fetch_test {switch | ndnName}*\n\
    -help     help\n\
    ...
    -nv       no resolve version (default: NDN_V_HIGH)\n";
...
main(int argc, string *argv) {
	...
	struct ndn_fetch *f = ndn_fetch_new(h);
	int ndnFD = ndn_get_connection_fd(h);
	int needHelp = ((argc < 2) ? 1 : 0);
	
	struct MyParms p = {0};
	
	p.resolveVersion = NDN_V_HIGH;
	p.f = f;
	p.ndnFD = ndnFD;
	p.maxSegs = 4;
	...
	while (i < argc) {
		...
		if (arg[0] == '-') {
			...
			if (p.debug != NULL)
				ndn_fetch_set_debug(f, p.debug, ndn_fetch_flags_NoteAll);
			p.src = arg;
			runTest(&p);
			p.appendOut = 1;
		}
