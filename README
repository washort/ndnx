...
For more information on the relationship between NDNx and CCNx, see
http://named-data.net/relationship-ndnx-ccnx/.
...
D. Minimal sample app (ndnChat) to demonstrate basic communication on
   local LAN.

E. Minimal sample file proxy (ndnFileProxy) to demonstrate basic
...
G. An Android implementation for smartphones.  The Android implementation
   has a service wrapper for ndnd and the repository.  It also has
   an NDN Chat implementation.
...
* `android/apps` - Android applications that use NDNx
* `android/apps/NDNx-Android-Chat` - The NDNx Chat application for Android
* `android/NDNx-Android-Lib` - A common Android Library for working with NDNx
* `android/NDNx-Android-Services` - Wrappers for ndnd and repository
* `android/external` - External libraries needed for ARM
* `apps` - experimental/sample apps tree
* `apps/ndnChat` - simple text chat sample in Java
* `apps/ndnFileProxy` - simple proxy making local files available via NDNx
* `apps/examples` - small example programs and code snippets
* `apps/HttpProxy` - sample HTTP proxy that converts HTTP Gets to NDN interests
* `apps/vlc` - vlc plugin for media transport experiments
* `apps/wireshark` - wireshark plugin dissector for decoding NDNx packets
* `csrc/` - C code tree
* `csrc/ndnd` - NDN daemon, the user-space forwarder implementation
...
* `csrc/contrib` - third-party library needed for certain platforms that
       are POSIX-deficient
* `csrc/include/ndn` - C header files
* `csrc/lib` - C application library implementation
* `csrc/libexec` - connectivity utilities, especially ndndc, the ndnd
...
* `javasrc/` - Java code tree.  The usual Java conventions are used for mapping
       package names to the file tree, with root package [org.ndnx.ndn](org.ndnx.ndn).
...
In addition, you will need vlc and wireshark to build and use the NDNx
...
For parts of the system written in Java, NDNx code is tested with Sun
...
defined: one in `javasrc` (Eclipse project name `NDNx-Java`) and one each
in `apps/ndnChat` (Eclipse project name `ndnChat`) and `apps/ndnFileProxy`
(Eclipse project name `ndnFileProxy`).  To import one of these projects
...
to eliminate it in the near future.  The NDNx distribution includes
...
See the notes in the Named Data Networking in C in the top-level
documentation.
...
* `ndnexplore` - primitive GUI browser in Java, usable to browse only
               repo content since it requires name enumeration support
               available only there.  Also allows writing files to repo.
* `ndnls` - primitive content listing by retrieval
* `ndnlsrepo` - command-line content listing from repo (via name 
                enumeration)
* `ndnputfile` - command-line tool to send file as NDNx data (asks repo
               to receive unless -raw arg given)
* `ndngetfile` - command-line tool to retrieve NDNx content and store it
               in file
* `ndnchat` - the ndnChat sample application
* `ndnfileproxy` - the ndnFileProxy sample application
