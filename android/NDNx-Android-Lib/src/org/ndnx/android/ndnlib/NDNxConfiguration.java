...
/**
 * Configure NDNx.  You should always run NDNxConfiguration.config(this) inside
 * your main application Activity before using any NDNx calls.
 */
public final class NDNxConfiguration {
	private final static String TAG = "NDNxConfiguration";

	/**
	 * Configure NDNx for Android with a shared user directory.
	 * @param ctx
	 */
	...
	/**
	 * Configure NDNx settings for Android.  The default is to use a shared
	 * user directory under /sdcard/ndnx/user.  If you want a private keystore
	 * then set privateUserDirectory to true
	 * 
	 * @param ctx
	 * @param privateUserDirectory true means keystore under /data/data/<pkg>/app_ndnx
	 */
	public static void config(Context ctx, boolean privateUserDirectory) {
		...
		if( privateUserDirectory || !Environment.getExternalStorageState().equalsIgnoreCase("mounted") ) {
			...
			root = ctx.getDir("ndnx", Context.MODE_PRIVATE);
		} else {
			root = Environment.getExternalStorageDirectory();
		}

		Log.i(TAG,"root storage = " + root.getAbsolutePath());
		File user = new File(root, "/ndnx/user");
